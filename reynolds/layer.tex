...
\subsection{Enthalpy.}  \label{ssec:enthalpy}  Suppose the ice is regarded as a mixture of solid-phase ice and liquid water, everywhere in local equilibrium, such that the amount of internal energy in a unit volume determines the temperature and the liquid water fraction.  The liquid water fraction is zero in cold ice and positive in temperate ice.  (The mixture theory which supports these glacier-specific internal energy concepts is laid out in \cite{Aschwandenetal2012}.)  Let $\phi=\rho \eta$ where $\rho$ is the mass density of the mixture and $\eta$ is the (specific) internal energy of the mixture with units of energy per mass, $\text{J}\,\text{kg}^{-1}$ in SI.  Thus $\phi=\rho\eta$ has units of energy-per-volume ($\text{J}\,\text{m}^{-3}$).  We call $\eta$ the (specific) enthalpy and $\phi$ the enthalpy density.  Note that enthalpy denotes all internal energy, excluding the kinetic energy.

Because the solid and liquid phases of water have different densities, and the mixture fraction varies in space and time, temperate ice is a variable-density fluid.  Both temperate and polythermal glaciers generally have variable density and variable liquid water fraction \cite{GreveBlatter2009}.  By contrast, everywhere-cold glaciers can be regarded as constant density and single phase, at least below the depth of any firn.

Consider the total enthalpy (internal energy)
\begin{equation}
E(t)=\int_{\Lambda(t)} \rho\eta\,dV, \label{eq:E}
\end{equation}
with units of Joules.  Applying Theorem \ref{thm:newreynolds} and free-surface equation \eqref{eq:ske} we find
\begin{equation}
