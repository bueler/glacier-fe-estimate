...
\title[Transport in glaciated regions]{Transport in glaciated regions}
...
\begin{abstract}
We consider the form that Reynold's transport theorem \cite{Holmes2019} takes for time-dependent 3D sets defined by a fixed lower surface $z=b(x,y)$ and a time-dependent upper surface $z=s(t,x,y)$, with $(x,y)$ in a fixed domain $\Omega\subset\RR^2$.  Thus we focus on the time-dependent spatial sets describing the glacier geometry within a glaciated region.  The 2D map-plane projection of such a 3D set, namely the time-dependent glacier extent, can have any outline or connectedness.  We assume that the 3D boundary of the glacier ice domain consists, up to a set of measure zero, only of these lower and upper surfaces; we neglect any cliffs along the margins of the glaciers.  The new form of the transport theorem is first applied to the evolution of total regional glacier mass, and mass changes, for general density fields.  That is, we make no assumption of constant ice density.  We also consider enthalpy, age, and linear momentum as examples.
\end{abstract}
...
Of the terms in the numerator in \eqref{eq:RDelta:reynolds}, the integral of the SMB $a$ over $\Pi(t)$ might be observed, or modeld via precipitation and an energy balance for melt processes.  The second integral in the numerator, over the ice base $\Xi(t)$, uses the trace of the 3D velocity $\bu$, presumably modeled by a stress balance for the glacier ice.  Note that there is no apparent conceptual benefit of rewriting the denominator in \eqref{eq:RDelta:reynolds} using the free-surface equation; we naturally regard $V'(t)$ as purely kinematical.


\section{Other applications}  \label{sec:otherapp}

In this section we apply Theorem \ref{thm:newreynolds} to the transport of fields other than mass density.

\subsection{Enthalpy.}  \label{ssec:enthalpy}  Suppose the ice is regarded as a mixture of solid-phase ice and liquid water, everywhere in local equilibrium, such that the amount of internal energy in a unit volume determines the temperature and the liquid water fraction.  Note that the liquid water fraction is zero in cold ice and positive in temperate ice.  The mixture theory which supports these glacier-specific internal energy concepts is laid out in \cite{Aschwandenetal2012}.
...
Let $\phi=\rho \eta$ where $\rho$ is the mass density of the mixture and $\eta$ is its (specific) internal energy, with units of energy per mass, $\text{J}\,\text{kg}^{-1}$ in SI.  Thus $\phi=\rho\eta$ has units of energy-per-volume ($\text{J}\,\text{m}^{-3}$).  We call $\eta$ the (specific) enthalpy\footnote{Reference \cite{Aschwandenetal2012} denotes the specific enthalpy by ``$H$'', but we use $\eta$ here because $h,H$ are used for ice thickness.} and $\phi$ the enthalpy density.  Note that enthalpy denotes all internal energy but it excludes the kinetic energy and the potential energy associated to gravity \cite{Aschwandenetal2012}.
