...
is the surface kinematical equation (SKE)
\item the time-dependent nonlinear complementarity problem (NCP),
\begin{align*}
s - b &\ge 0 &&\phantom{x} \\
\frac{\partial s}{\partial t} - \bu|_s \cdot \bn_s - a &\ge 0 \\
(s - b) \left(\frac{\partial s}{\partial t} - \bu|_s \cdot \bn_s - a\right) &= 0
\end{align*}
is the underlying \emph{free-boundary} meaning of the SKE, whatever the dynamical model within the ice
\item this NCP first appears in (Calvo et al 2003), but only for the shallow ice approximation
\end{itemize}
\end{frame}
