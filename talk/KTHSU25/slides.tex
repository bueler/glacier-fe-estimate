...
\end{center}
    ...
    \begin{itemize}
    \item[$\circ$] what is the \aler{geometry}, namely the surface elevation, of the ice?
    \end{itemize}
...
\item for a unilateral obstacle problem, with $\cK = \{v\in\cX\,:\,v\ge \psi\}$, the residual $F(u)-\ell$ is at least \aler{nonnegative}
    \begin{itemize}
    \item[$\circ$] for classical obstacle problem:
...
\vspace{-25mm}
\hfill \only<2>{\input{tikz/nonadmissible.tex}}
\end{frame}
...
\begin{align*}
\|u-u_h\|^\qq &\le \frac{2}{\alpha} \left(\inf_{v\in\cK} \left(F(u)-\ell\right)[v-u_h] + \inf_{v_h\in\cK_h} \left(F(u)-\ell\right)[v_h-u]\right) \\
   &\quad\, + \frac{2}{\alpha} \left(F(u_h)-F_h(u_h)\right)[u_h] + c \inf_{v_h\in\cK_h} \|v_h - u\|^{\qq'}
\end{align*}
\end{thm}

\begin{itemize}
\only<1>{\item in the unconstrained case ($\cK=\cX$ and $\cK_h=\cX_h$), with no variational crimes ($F_h=F$), this is Cea's lemma in a Banach space:
    $$\|u-u_h\|^\qq \le c \inf_{v_h\in\cK_h} \|v_h - u\|^{\qq'}$$

\vspace{5.5mm}
\phantom{foo}}
\only<2>{\item if $\cX$ is a Hilbert space, $F(u)[v]=(Au,v)$ is linear, $\cX \subset \cH$ for some Hilbert space $\cH$, $\qq=2$, and with no variational crimes ($F_h=F$), then this is Falk's (1974) theorem:
{\small
\begin{align*}
...
\begin{itemize}
\item an obstacle problem, here in complementarity problem form, holds in $[0,T] \times \Omega$:
\begin{align*}
s - b &\ge 0 &&\phantom{x} \\
...
\begin{itemize}
\item define:
    $$\Lambda(t) = \{(x,z)\,:\,b(x)<z<s(t,x)\} \hspace{20mm}$$
\item Glen-Stokes equations in $\Lambda(t)$ with $\pp = (\nn + 1)/\nn \approx 4/3$:
\begin{align*}
- \nabla \cdot \left(2 \nu(D\bu)\, D\bu\right) + \nabla p &= \rhoi \bg  \\
\nabla \cdot \bu &= 0  \\
\nu(D\bu) &= \nu_0 |D\bu|^{\pp-2} && \phantom{foo}
\end{align*}
...
\begin{smodel}
a complementarity problem (obstacle problem) in $[0,T]\times \Omega$, for fixed $\Omega \subset \RR^2$, coupled to a Glen-Stokes problem within the ice $\Lambda(t) \subset \RR^3$
\end{smodel}
\end{frame}
