...
\item the intuitive/naive strong form poses the problem in terms of its solution (\aler{nonsense}):
    $$-\grad^2 u = \varphi \quad \text{ on $R_u$, \qquad and \qquad} u = \psi \quad \text{ on $A_u$}$$
\item the \aler{nonlinear complementarity problem} strong form has sensible statements everywhere on $\Omega$:
...
\vspace{-25mm}
\hfill \input{tikz/nonadmissible.tex}
\end{frame}
...
\begin{itemize}
\item<2> in the unconstrained case ($\cK=\cX$ and $\cK_h=\cX_h$) with no variational crimes ($F_h=F$), then this is Cea's lemma in a Banach space:
