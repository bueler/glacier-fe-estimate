...
\item if $\mathcal{K}$ is nontrivial, VI($F$,$\mathcal{K}$) is nonlinear, even when $F$ is linear
    \begin{itemize}
    \item[$\circ$] classical obstacle problem: \, $F(v)[w] = \int_\Omega \grad v\cdot \grad w - \varphi w$
    \end{itemize}
\onslide<2>{\begin{theorem}
if $F$ is coercive then there is a unique $u\in\cK \subset H^1(\Omega)$ solving VI($F$,$\mathcal{K}$)
\end{theorem}}
\end{itemize}
\end{frame}
