...
\begin{itemize}
\only<1>{\item in the unconstrained case ($\cK=\cX$), with no variational crimes ($F_h=F$), this is \aler{Cea's lemma} in a Banach space:
...
\only<3>{\item for unilateral \aler{obstacle problems} $\cK = \{v\ge \psi\}$, with $d\mu_u=F(u)-\ell$ a \aler{positive measure supported in the exact active set $A_u$}, this says
\begin{align*}
