...
\item $\bu|_s(t,x)$ surface value of ice velocity solution, extended by zero to bare land
\end{itemize}
\end{frame}
...
\only<1>{
\begin{smodel}[a suggested name]
an \alert{NCP} in $[0,T]\times \Omega$, for fixed domain $\Omega \subset \RR^2$, \alert{coupled to Glen-Stokes within the ice}, in a time-dependent domain $\Lambda(s) \subset \RR^3$
\end{smodel}
}
\only<2>{
...
\begin{itemize}
\item because DAE systems are infinitely stiff, implicit time discretization is expected
    \begin{itemize}
    \item[$\circ$] stiff decay $n=1,2,3$-order BDF schemes, or Radau schemes
    \end{itemize}
...
the weak form \emph{backward Euler time-step problem}, for $t_k \in [0,T]$, is to find the surface elevation $s^k \approx s(t_k,x) \in \cK$ solving the VI
$$F(s^k)[r-s^k] \ge \ell^k[r-s^k] \quad \text{for all } r \in \cK$$
\end{block}
}
\only<2>{
\begin{block}{the main idea}
the weak form \emph{backward Euler time-step problem}, for $t_k \in [0,T]$, is to figure out the map-plane domain covered by the glacier, and simultaneously to compute the updated surface elevation $s^k$
\end{block}
...
\begin{frame}{but maybe you came for the finite elements?}

\begin{itemize}
\item why would we care about well-posedness?
\item recall the canonical FE story about convergence:
{\small
\begin{align*}
-\grad^2 \alert{u} &= f \\
&\downarrow & &\text{\emph{to weak form}} \\
a(\alert{u},v) &= \ell_f(v) \\
&\downarrow & &\text{\emph{finite elements}} \\
a(u_h,v_h) &= \ell_f(v_h) \\
...
\item well-posedness for $\alert{u}$ must be either proven, conjectured, or politely ignored, if you want to tell this story for glacier surface elevations
    \begin{itemize}
    \item[$\circ$] I am choosing ``conjectured''
    \end{itemize}
\end{itemize}
\end{frame}
