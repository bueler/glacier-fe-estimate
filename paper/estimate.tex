...
Each term in estimate \eqref{eq:preglacierestimate} has a glaciological meaning, exposed in the following theorem.  Recall that $\gamma_\pp(\Lambda_{s_h})$ denotes the trace constant (Lemma \ref{lem:trace}) for the domain defined by the surface $s_h$.  Recall that $h$ denotes the maximum diameter of a mesh cell over $\Omega$, while the norm $\|\cdot\|_\cV$ is defined in \eqref{eq:vnorm}.

\begin{theorem} \label{thm:glacierapp} Make the same assumptions as in Lemma \ref{lem:preglacierapp}.  Define $\Omega_A(s) = \left\{x\in\Omega\,:\,s(x)=b(x)\right\}$.  With $c_0>0$ from the Lemma, there exists $c_2>0$ independent of $s,s_h$ such that

FIXME: ALL TERMS NEED CHECKING FROM PROOF
\begin{align}
\|s-s_h\|_\cX^\rr &\le \quad \frac{2}{\alpha} \int_{\Omega_A(s)} (b - \ell^n) (b_h - b) &&\text{[term 1]} \label{eq:glacierestimate} \\
   &\quad\, + \frac{\Delta t}{\alpha} \, \Gamma(s_h) \big\|\bu_h - \bu\big\|_{\cV} &&\text{[term 2]} \notag \\
   &\quad\, + c_0 c_2 h \left(\|s\|_{W^{2,\rr}(\Omega)}\right)^\qq &&\text{[term 3]} \notag
\end{align}
where
\begin{equation}
...
\medskip
\begin{itemize}
\item[\emph{[term 1]:}] This term arise from the approximation of the bed in the ice-free area.\footnote{That is, in the ice-free area \emph{according to the exact solution} $s$.}  If the bed happens to be exactly representable on the mesh ($b_h=b$) then it is zero.  Because the meshed bed $b_h$ is above the exact bed $b$, $b_h$ stops $s_h$ from having the correct values $s=b$ in the ice-free area, and this is the smallest such error.  Note that $b_h\ge b$ and $b-\ell^n\ge 0$---see the argument in Section \ref{sec:model}---in this case, so the integrand is positive.  The presence of the coercivity constant $\alpha$ the set $\Omega_A(s)$ in this term, shows it relates to the ice flow in a subtle manner.
\item[\emph{[term 2]:}] This term shows how numerical errors in the numerical solution of the Stokes equations, i.e.~over the particular domain $\Lambda_{s_h}$, affect the error in $s_h$ itself.  For example, if a higher-order FE method is applied to the Stokes problem \eqref{eq:stokes}, so that $\big\|\bu_h - \bu\big\|_{\cV} = O(h^k)$, then this will be reflected in the convergence rate in this term.
\item[\emph{[term 3]:}] This term arises from the classical interpolation-error argument for convergence of FE methods, but with the added complication that a $P^1$ interpolant of $s$ must be truncated into $\cK_h$ before the element-wise interpolation argument can be applied.  (See the proof.)
\end{itemize}
\begin{proof}
FIXME
\end{proof}
