...
\subsection{Weak form} \label{subsec:weakform} With the above tools we can now define an operator which puts the backward Euler time step VI \eqref{eq:be:viearly} into a clean and mathematically-precise weak form.  If $s\in\cK$ and $q\in\cX$ then we define $F_{\Delta t}:\cK\to\cX'$:
\begin{equation}
...
FIXME show what follows from (3.23): 1. SKE, 2. inequality in $\{s=b\}$, esp $b - \ell^n\ge 0$, 3. 1D condition at margin?
...
\section{Numerical exploration of Conjectures \ref{conj:a} and \ref{conj:b}} \label{sec:numerical}

FIXME using Firedrake \cite{Hametal2023}
...
If $f:\cK \to \cX'$ is monotone and coercive, and also continuous on finite-dimensional subspaces, then VI \eqref{eq:vi} has a solution \cite[Corollary III.1.8]{KinderlehrerStampacchia1980}.  If $f$ is strictly monotone then the solution is unique.  If $f$ is $\qq$-coercive then it coercive and strictly monotone, so $\qq$-coercivity and continuity yield well-posedness for \eqref{eq:vi}.  Note that Definitions \ref{def:monotonecoercive} and \ref{def:lipshitz} do not require $f$ to be defined on all of the Banach space $\cX$, but only on $\cK$.  
...
An FE method for \eqref{eq:vi} is a finite-dimensional VI problem.  Suppose $\cX_h \subset \cX$ is a finite-dimensional subspace, typically some space of continuous, piecewise-polynomial functions defined on a mesh.  The FE constraint set $\cK_h\subset \cX_h$ is assumed to be closed and convex, but generally $\cK_h \nsubseteq \cK$.  Let $f_h:\cK_h\to\cX'$, but note that generally $f_h\ne f$ because of quadrature and other approximations.  Looking ahead, both $\cK_h \nsubseteq \cK$ and $f_h\ne f$ occur naturally in the glacier geometry problem (Section \ref{sec:application}).

The FE VI problem is
\begin{equation}
f_h(u_h)[v_h-u_h] \ge \ell[v_h-u_h] \quad \text{for all } v_h\in \cK_h, \label{eq:fe:vi}
\end{equation}
just like \eqref{eq:vi}.  We will assume that \eqref{eq:fe:vi} has a solution $u_h\in\cK_h$.

The following abstract error estimation theorem extends the well-known result by \cite{Falk1974}; see also Theorem 5.1.1 in \cite{Ciarlet2002}.  We will not assume that $f$ is linear, nor that $\cK_h \subset \cK$ or $f_h=f$, nor that $f_h$ is $\qq$-coercive, or Lipschitz, or even continuous.  However, we must assume that the domain of $f$ includes the FE solution, which is achieved here\footnote{This technical assumption permits a clean and general estimation theorem, but the choice of $\cK_h$ made in Section \ref{sec:application} means that $\hcK$ is not needed in our glacier application.  See Corollary \ref{cor:abstractestimate:nohull}.} by defining a convex superset of $\cK$ and $\cK_h$.
...
There are two important cases where the convex hull operation to create $\hcK$ is not needed, as in this Corollary.  We will see in Section \ref{sec:application} that case \emph{i)} can easily be imposed in glacier simulations.
...
\end{enumerate}
Then, without the construction of the convex hull $\hcK$, conclusion \eqref{eq:abstractestimate} applies as stated, except that in case i) the ``\,$\inf_{v\in\cK}$'' term in \eqref{eq:abstractestimate} is zero.
\end{corollary}

Consider $f(u)-\ell\in \cX'$.  It might be a measure or a measurable function, and the first two terms in estimate \eqref{eq:abstractestimate} preserve information about its support, which plays an important role in the glacier application of Section \ref{sec:application}.  By contrast, the Hilbert space result by Falk \cite{Falk1974}, \cite[section 5.1]{Ciarlet2002} computes norms and loses this information.  The following Corollary, with easy proof, takes such a norm-based approach.  We suppose that $\cX$ continuously and densely embeds into a larger Banach space $\cB$:
\begin{equation}
...
In fact the precise result by Falk \cite{Falk1974} combines the above two Corollaries, and the further assumption that $f$ is linear.  Suppose $f(v)[w]=a(v,w)$ is bilinear, uniformly elliptic, and continuous on a Hilbert space $\cX$.  (The definition of uniformly elliptic coincides with definition \eqref{eq:qcoercive} of $2$-coercive, and continuity of $a(v,w)$ implies \eqref{eq:liponbounded}.)  Suppose that $\cX\hookrightarrow \cH$ and $\bar{\cX} = \cH$ for some Hilbert space $\cH$, and that $\|f(u)-\ell\|_{\cH'} < \infty$ so that, up to isomorphism, $f(u)-\ell \in \cH$.  Finally, suppose that $f(u_h)=f_h(u_h)$.  Then case \emph{ii)} of Corollary \ref{cor:abstractestimate:nohull} combines with Corollary \ref{cor:abstractestimate:Bnorm} to yield Theorem 1 in \cite{Falk1974}, and also Theorem 5.1.1 in \cite{Ciarlet2002}.

The quantity in estimates \eqref{eq:abstractestimate} and \eqref{eq:abstractestimate:Bnorm} which involves $v-u_h$ and $\inf_{v\in\cK}$ is generally nonzero in obstacle problems where $\cK_h \nsubseteq \cK$ \cite{Ciarlet2002}.  In fact, consider a unilateral obstacle problem where $\cK=\{v \in \cX\,:\,v\ge \psi\}$.  Suppose $\psi_h$ is an FE interpolant of $\psi$, and define $\cK_h=\{v_h \in \cX_h\,:\,v_h\ge \psi_h\}$.  Observe that, while $\psi_h(x_j)=\psi(x_j)$ for interpolation nodes $x_j$, generally $\psi_h(x) \ge \psi(x)$ does not hold for all $x\in\Omega$, even if $\psi$ is arbitrarily smooth.  In other words, nodal admissiblity does not imply admissibility (Figure \ref{fig:nonadmissible}) when interpolation is applied to the obstacle.

\begin{figure}[ht]
\begin{center}
FIXME figure like Ciarlet Figure 5.1.3 %\includegraphics[width=0.8\textwidth]{figs/xxx.jpg}
\end{center}
\caption{Nodal admissibility generally does not imply admissibility.  When $\psi_h$ is an FE interpolant of an obstacle $\psi$, it is possible that $\cK_h \nsubseteq \cK$.}
\label{fig:nonadmissible}
\end{figure}

This observation is important for glacier models tracking the surface elevation, but in Section \ref{sec:application} we will see that a one-sided interpolation process allows us to bypass this concern.  Models which solve for ice thickness avoid it because then $\cK = \{v\in\cX\,:\,v\ge 0\}$, and generally $\cK_h=\cK\cap\cX_h \subset \cK$ holds in the FE method.

The following Corollary collects various conclusions one might draw from assuming $\cK_h \subset \cK$ and then making stronger assumptions, especially that $f_h=f$, at least on $u_h$.  We also connect to the familiar PDE case.
...
\end{equation}
On the other hand, if $f(u)=\ell$, for example if $u$ is in the interior of $\cK$, then
\begin{equation}
\|u-u_h\|^\qq \le c \inf_{v_h\in\cK_h} \|v_h-u\|^{\qq'} \label{eq:abstractestimate:subset:Cea}
\end{equation}
\end{corollary}

Note that \eqref{eq:abstractestimate:subset:Cea} is Cea's lemma \cite[Theorem 2.4.1]{Ciarlet2002} in a Banach space, though with a constant which depends on $R_h=\max\{\|u\|,\|u_h\|\}$.  This case is a reduction to the PDE case where there is no active set or free boundary.  In the glacier context it applies when the entire domain $\Omega$ is covered in ice.
...
The numerical velocity result from the Stokes problem shown in the Figure, resulting from the geometry defined by $s_h$, is denoted $\bu_h$, and its surface trace is denoted $\bu_h|_{s_h}$.  Observe that $\bu_h|_{s_h}$ will generally be different from the trace of the exact solution of the same Stokes boundary value problem for the same geometry.  This exact solution will simply be denoted by $\bu$, and its surface trace by $\bu|_{s_h}$.  Although $F^h_{\Delta t}(s_h)$ is defined by formula \eqref{eq:be:Fdefine}, it is a different operator from $F_{\Delta t}(s_h)$ because it uses the numerical surface velocity trace $\bu_h|_{s_h}$, for the given geometry, and not the trace of the exact solution of that problem, $\bu|_{s_h}$.

A key concern in applying abstract FE Theorem \ref{thm:abstractestimate}, or its Corollaries, is the choice of the numerical bed elevation $b_h \approx b$, which defines the constraint set $\cK_h$.  In the glaciological context one may assume $b$ is continuous on the closed domain $\bar\Omega$.  An abstract $b\in C(\bar\Omega) \cap \cX$ can be considered, but in practice $b$ will be provided via a high resolution map derived from e.g.~ice-penetrating radar \cite{Morlighemetal2017}, thus one may suppose $b$ is itself in an FE space, but on a (much) finer mesh.  We assert that, because of what we prove below about the solution to \eqref{eq:fe:be:vi}, $b_h \in \cX_h$ should be chosen to satisfy $b_h\ge b$.  To achieve this maximum monotone restriction \cite{BuelerFarrell2024} or similar can be applied, that is, $b_h = R^\oplus b$ or $b_h = (R^\oplus)^k b$, at least if $\cX_h$ is from a submesh of the fine data mesh on which $b$ is provided.

We will also assume that $b|_{\partial\Omega}$ is piecewise-polynomial, as one would in ``conforming'' FE methods for classical PDE problems \cite{Elmanetal2014}.  Furthermore we will need an interpolation/truncation operation $\Pi_h : \cX \to \cK_h$.  When applied to $r\in\cX$ this operation gives the unique admissible FE function $\Pi_h(r) \in \cK_h$ so that
\begin{equation}
\Pi_h(r)(x_j) = \max \,\{b_h(x_j), r(x_j)\} \label{eq:definePi}
\end{equation}
for every node $x_j \in \cT_h$, and $\Pi_h(x_j)=b(x_j)$ if $x_j\in\partial\Omega$.  Observe, however, that nodal interpolation and truncation in definition \eqref{eq:definePi} only yield nodal admissibility $\Pi_h(r)(x_j) \ge b_h(x_j)$.  The FE space must be such that this implies admissibility \emph{per se}, namely $\Pi_h(r)(x) \ge b_h(x)$ for all $x \in \Omega$.  This condition is satisfied by the continuous and piecewise-linear FE space $P_1$, but not, for example, by $P_2$ \cite{BuelerFarrell2024}.
...
\begin{proof}
Theorem \ref{thm:stepwellposed} shows that $s$ is well-defined.  Then a straightforward application of Corollary \ref{cor:abstractestimate:nohull} in case \emph{i)}, using formula \eqref{eq:be:Fdefine} to define $F_{\Delta t}$, gives \eqref{eq:preglacierestimate}.
\end{proof}

Each term in estimate \eqref{eq:preglacierestimate} has a glaciological meaning, which we expose in the following Theorem.  For this purpose recall that $\gamma_\pp(\Lambda_{s_h})$ denotes the trace constant (Lemma \ref{lem:trace}) for the domain defined by the surface $s_h$, $h$ denotes the maximum diameter of a mesh cell over $\Omega$, $\cV=W_b^{1,\pp}(\Lambda_{s_h}; \RR^3)$ is the velocity space for the Stokes weak-form problem \eqref{eq:glenstokes:weak}, and $\|\cdot\|_\cV$ is the norm defined in \eqref{eq:vnorm}.

\begin{theorem} \label{thm:glacierapp} Make the same assumptions as in Lemma \ref{lem:preglacierapp}.  Define $\Omega_A(s) = \left\{x\in\Omega\,:\,s(x)=b(x)\right\}$, a measurable subset of $\Omega$.  Then
\begin{align}
\phantom{dfkljsd} \|s-s_h\|_\cX^\rr &\le \quad \frac{2}{\alpha} \int_{\Omega_A(s)} (b - \ell^n) (b_h - b) &&\text{\textnormal{[term 1]}} \label{eq:glacierestimate} \\
...
\medskip
\begin{itemize}
\item[term 1:] This error comes from approximation of the bed in the ice-free domain (as defined by the exact solution $s$).  If the bed is exactly representable $b_h=b$ then this term is zero, but generally $b_h\ge b$ and $b-\ell^n\ge 0$---see the argument in Section \ref{sec:model}---so the integrand is positive.  Note that $s_h \ge b_h \ge b = s$ in the ice-free area, so the factor $b_h-b$ in the integrand reflects the smallest error $s_h \approx s$ which is possible here.  The presence of the coercivity constant $\alpha$ in this term shows that it relates to the ice flow.

\item[term 2:] This shows how numerical errors in the numerical solution of the Stokes equations, over the domain $\Lambda_{s_h}$, affect the error in $s_h$.  For an FE method applied to the Stokes problem \eqref{eq:stokes} with some convergence rate $\|\bu_h - \bu\|_{\cV} = O(h^k)$, this term would reflect that rate.

\item[term 3:] A similar expression arises in the classical Cea's lemma argument for convergence of FE methods \cite{Ciarlet2002}, but here there is an added complication.  The interpolant of $s$ must be truncated into $\cK_h$, and nodal admissibility must imply admissibility.
\end{itemize}

\begin{proof}
Consider the first term on the right side of inequality \eqref{eq:preglacierestimate}.  Because $s$ solves \eqref{eq:be:vi}, the residual $\Psi = F_{\Delta t}(s)-\ell^n \in \cX'$, while generally nonzero, is non-negative.  In particular, if $\phi\in C_c^\infty(\Omega)$ is nonnegative then $r=s+\phi \in \cK$ and $\Psi[r-s] = \Psi[\phi] \ge 0$.  Thus $\Psi\in\cX'$ is a non-negative distribution, and so it is represented by a positive Borel measure $\mu$, $\Psi[\phi] = \int_\Omega \phi\,d\mu$ \cite[Theorem 6.22]{LiebLoss1997}.  However, by the proof of Theorem II.6.9 in \cite{KinderlehrerStampacchia1980} this measure is supported in $\Omega_A(s)$.  In fact, recall from Section \ref{sec:model} that $b-\ell^n\ge 0$ on $\Omega_A(s)$---this gives the density of $\mu$--and note also that $\bu|_{s}=\bzero$ and $s=b$ on $\Omega_A(s)$.  Suppose $\phi_h \in \cX_h$ is nonnegative and supported in $\Omega_A(s)$, and define $r_h = b_h + \phi_h \in \cK_h$.  Then from definition \eqref{eq:be:Fdefine} we have
\begin{equation}
\left(F_{\Delta t}(s)-\ell^n\right)[r_h-s] = \int_{\Omega_A(s)} \left(b - \ell^n\right) (b_h - b) + \int_{\Omega_A(s)} \left(b - \ell^n\right) \phi_h.
\end{equation}
Therefore set $\phi_h=0$ to derive term 1:
\begin{equation}
\frac{2}{\alpha} \inf_{r_h\in\cK_h} \left(F_{\Delta t}(s)-\ell^n\right)[r_h-s] \le \frac{2}{\alpha}  \int_{\Omega_A(s)} \left(b - \ell^n\right) (b_h - b).
\end{equation}
...
\end{align}
Recalling the scaling of the $\cX$ norm, equation \eqref{eq:norm:Omega}, we have term 2.

Term 3 follows by substituting $r_h=\Pi_h(s)$.  (This uses item \ref{item:Pi} in the Standard Assumptions.)
\end{proof}

FIXME SUSPICIOUSLY STRONG REGULARITY ASSUMPTION ON $s$, AND BELIEVABLE ON $\bu$ ALLOW US TO DEAL WITH terms 2,3; USE RESULT IN \cite{JouvetBueler2012}, namely \cite[Theorem 3.1.6]{Ciarlet2002}; term 1 WOULD SEEM TO REQUIRE FURTHER THEORY FOR HOW REGULARITY HELPS WITH $R^\oplus$


\section{Discussion and conclusion} \label{sec:conclusion}

FIXME the barrier theory from \cite{Bueler2021conservation} applies to thickness-based models; term 1 in Theorem \ref{thm:glacierapp} is novel relative to that

FIXME a weaker version of Conjecture \ref{conj:b}, which does not address the marginal overhang issue, may be easier to prove, but it is not sufficient for Theorem \ref{thm:stepwellposed}; it comes from setting $r=s + \phi$ for $\phi$ supported where $s>b$
