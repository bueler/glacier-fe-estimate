...
\begin{equation}
\left(f(v)-f(w)\right)[v-w] \ge \alpha \|v-w\|^\qq \qquad \text{for all } v,w \in \cK. \label{eq:pcoercive}
\end{equation}
\end{definition}
...
The following definition appeared in Lemma \ref{lem:philipschitz}.  Note that Definitions \ref{def:monotonepcoercive} and \ref{def:lipshitz} do not require $f$ to be defined on all of the vector space $\cX$, but only on $\cK$.  If Definition \ref{def:lipshitz} applies then $f$ is continuous.
...
Consider $f(u)-\ell\in \cX'$ in Theorem \ref{thm:abstractestimate}.  In practical applications it might be a signed measure or a real-valued measurable function.  In such cases, the first two terms in estimate \eqref{eq:abstractestimate} include information about the sign of $f(u)-\ell$.  By contrast, the original Hilbert space result (\cite{Falk1974} and \cite[section 5.1]{Ciarlet2002}) compute norms and lose this information.  The following corollary to Theorem \ref{thm:abstractestimate} illustrates that norm-based approach.  We suppose that $\cX$ continuously and densely embeds in a larger Banach space $\cB$:
\begin{equation}
\cX \hookrightarrow \cB, \quad \overline{\cX} = \cB \label{eq:VembedsinB}
\end{equation}
Observe that $\cB' \subset \cX'$.  A standard example is where $\cX=W^{1,\qq}(\Omega)$ and $\cB=L^\qq(\Omega)$.  The proof of the following is immediate.
...
The convex hull operation \eqref{eq:convexhull} is not needed if the nonlinear operator $f$ is defined on all of $\cX$ and if $f_h=f$.  That is, \eqref{eq:abstractestimate} holds if one replaces $\hcK$ with $\cX$ itself for the domain of $f$.  Furthermore, for \emph{linear} operators defined in this way, Theorem \ref{thm:abstractestimate} reduces to the FE error estimate for VIs by Falk \cite{Falk1974}.  Specifically, suppose $\ip{f(v)}{w}=a(v,w)$ is actually bilinear, $\cX$-elliptic, and continuous on a Hilbert space $\cX$.  Define $A:\cX\to\cX'$, a bounded linear operator, by $Av(w) = a(v,w)$.  Observe that continuity for $a(v,w)$ implies Lipshitz on bounded sets \eqref{eq:liponbounded} for $A$.  Suppose that $\cX\hookrightarrow \cH$ and $\overline{\cX} = \cH$ for some larger Hilbert space $\cH$, and that $\|Au-\ell\|_{\cH'} < \infty$ so that, up to isomorphism, $Au-\ell \in \cH$.  Then Corollary \ref{cor:abstractestimate:Bnorm} reduces to Theorem 1 in \cite{Falk1974} and Theorem 5.1.1 in \cite{Ciarlet2002}.

As observed in \cite{Ciarlet2002}, the quantity in estimates \eqref{eq:abstractestimate} and \eqref{eq:abstractestimate:Bnorm} which involves $v-u_h$, for $v\in\cK$, is generally nonzero in obstacle problems where $\cK_h \nsubseteq \cK$.  (This is relevant to glacier models tracking the surface elevation, as opposed to thickness.)  In fact, suppose that $\cK=\{v \in \cX\,:\,v\ge \psi\}$, $\psi_h$ is an interpolant of $\psi$, and $\cK_h=\{v_h \in \cX_h\,:\,v_h\ge \psi_h\}$.  Observe that, while $\psi_h(x_j)=\psi(x_j)$ for interpolation nodes $x_j$, generally $\psi_h(x) \ge \psi(x)$ does not hold for all $x\in\Omega$, even if $\psi$ is arbitrarily smooth.  In other words, nodal admissiblity does not imply admissibility (Figure \ref{fig:nonadmissible}).
...
The following corollary is relevant to glacier models solving for ice thickness, as opposed to surface elevation.  In such cases $\cK = \{v\ge 0\}$ consists of all nonnegative functions and $\cK_h=\cK\cap\cX_h$.
