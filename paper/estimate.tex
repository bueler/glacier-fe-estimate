...
Combining Lemmas \ref{lem:stokesapriori} and \ref{lem:trace} yields the following bound on the surface trace of the velocity, which justifies the use of notation ``$\bu|_{s}$'' when $\bu$ solves the glaciological Stokes model.  When applying this result, recall that $\Lambda=\Lambda(s)$ and $\Gamma_s=\partial \Lambda\setminus \Gamma_b$ are defined, in terms of $s$ and $b$, by \eqref{eq:icydomain}.
...
Now we return to implicit time steps for glacier evolution.  Let $\{t_n\}$ be any increasing sequence of times in $[0,T]$, with $t_0=0$.  Let $\Delta t = t_n-t_{n-1}$ denote a step duration, and let $a^n(x)$ be the (temporal) average of the data $a(t,x)$ over $[t_{n-1},t_n]$.  Suppose that $s^n(x)\approx s(t_n,x)$ approximates the surface elevation at time $t_n$.  Using a backward Euler step \cite{AscherPetzold1998}, SKE \eqref{eq:ske} becomes
\begin{equation}
...
For cleaner appearance we write $s=s^n$.  Also, we collect a source term, defined over all of $\Omega$, which has units of elevation:
\begin{equation}
\boxed{\ell^n(x) = s^{n-1}(x)+\Delta t\,a^n(x) = s^{n-1}(x) + \int_{t_{n-1}}^{t_n} a(t,x)\,dt.} \label{eq:be:source}
\end{equation}

As noted in the Introduction, $s=s^n$ in \eqref{eq:be:ske} actually solves a problem of free-boundary type, namely NCP \eqref{eq:be:ncp}, which says that, at the solution time $t_n$, either there is bare ground ($s=b$) or equation \eqref{eq:be:ske} holds; while $s$ does not solve \eqref{eq:be:ske} over the ice-free parts of $\Omega \subset \RR^2$, it solves \eqref{eq:be:ncp}.  However, the strong form \eqref{eq:be:ncp} has a weak-form variational inequality (VI; \cite{Evans2010,KinderlehrerStampacchia1980}) version, as follows, which is better-suited to both well-posedness theory and finite element (FE) analysis.

Regarding the precise Banach space $\cX$ of surface elevations as momentarily undecided, admissible surface elevations for the weak formulation come from a convex and closed subset,
\begin{equation}
\boxed{\cK = \left\{r \in\cX\,:\,r|_{\partial\Omega}=b|_{\partial\Omega} \text{ and } r \ge b\right\};}  \label{eq:be:admissible}
\end{equation}
note this includes the fixed (Dirichlet) boundary condition.  The VI is then derived by assuming that $s \in \cK$ is a sufficiently-regular solution of \eqref{eq:be:ncp} \cite{Bueler2021conservation}.  Let $\Omega_I$ be the (measurable) subset of $\Omega$ on which the $s\ge b$ constraint is inactive, where glacier ice is present: $\Omega_I = \{x\,:\,s(x)>b(x)\}$.  Integration over $\Omega_I$ then shows that
\begin{equation}
\int_{\Omega_I} \left(s - \Delta t\,\bu|_s \cdot \bn_s - \ell^n\right)\,(r-s) = 0  \label{eq:inactivetruth}
\end{equation}
for any $r\in\cK$.  On the other hand, let $\Omega_A = \{x \in \Omega \,:\,s(x)=b(x)\}$ be the complementary active (ice-free) region.  Using extension by zero \eqref{eq:defineus}, note that \eqref{eq:be:ncp:residualpos} implies $b-\ell^n = s - \Delta t\,\bu|_s \cdot \bn_s - \ell^n \ge 0$ on $\Omega_A$.  Since also $r-s=r-b\ge 0$ on $\Omega_A$, if $r\in\cK$, integration yields an inequality:
\begin{equation}
...
\end{equation}
Almost everywhere, land is in $\Omega_I$ or in $\Omega_A$, so addition of \eqref{eq:inactivetruth} and \eqref{eq:activetruth} gives the following VI for $s \in \cK$:
\begin{equation}
\int_\Omega \left(s - \Delta t\,\bu|_s \cdot \bn_s - \ell^n\right)\,(r-s) \ge 0 \quad \text{for all } r \in \cK. \label{eq:be:viearly}
\end{equation}
This is true for $s\in\cK$, in advance of knowing which part of $\Omega$ is the ice-covered.

It remains to identify a Sobolev space $\cX$ suitable for the admissible set $\cK$ of surface elevations.  However, within a Stokes-based theory the shape that should be predicted for a glacier's grounded margin is not clear (Figure \ref{fig:margins}), and this situation makes it difficult to pin down $\cX$.  Shallow-ice theory suggests root-type (fractional-power) shapes for the marginal surface elevation, with different shapes for advance and retreat but unbounded gradients \cite{Bueleretal2005,JouvetBueler2012}.  ``Wedge'' shapes with bounded gradients have also been hypothesized \cite{EchelmeyerKamb1986}, which would allow $s\in W^{1,\rr}(\Omega)$.
...
Reality is more complicated.  In the vicinity of an ice margin, especially on steep bedrock features, real glacier ice can generate overhangs which violate the assumption of a single-valued surface elevation function, and the bedrock itself can overhang.  Fractures, crevasses, and cliffs are also commonly found in glacier margins, but modeling such features would depart from our viscous fluid paradigm.  However, because fractured margins are small compared to the overall scale of glaciers and ice sheets, modeling literature generally ignores overhangs, and it assumes instead that surface and bed elevation functions are well-defined  \cite{IsaacStadlerGhattas2015,Jouvetetal2008,LofgrenAhlkronaHelanow2022,WirbelJarosch2020}.  One might allow fractures, for example by supplementing momentum conservation with an additional advected damage variable \cite{PralongFunk2005}, so that ice-cliff calving occurs via a stress-failure criterion.  The resulting emergent, spatially-averaged margin shapes might then suggest an appropriate regularity assumption and or Sobolev space for surface elevations, within a viscous-only theory, but such a construction is well outside the scope of the current work.

For now we attempt to choose $\cX$ based on mathematical considerations within the viscous theory.  Observe that well-posedness of the weak-form Stokes problem \eqref{eq:glenstokes:weak} over a 3D domain $\Lambda(s)$, plus the surface trace bound in Corollary \ref{cor:surfacetracebound}, allows us to create a well-defined map from an admissible surface elevation $s$ to the surface motion term $\bu|_s\cdot \bn_s$ in \eqref{eq:be:viearly}.  The map is defined via \eqref{eq:icydomain} for $\Lambda(s)$, followed by the solution of the Stokes model \eqref{eq:glenstokes:weak} over $\Lambda(s)$, then evaluation of the trace of $\bu$ along $\Gamma_s$ (Corollary \ref{cor:surfacetracebound}), definition \eqref{eq:defineus} which extends by zero, and finally multiplication by $\bn_s$.  Evidently, for this to be defined $s$ must be admissible ($s\ge b$) and sufficiently-regular, compatible with the choice of $\cX$.

In fact, we can bound the surface motion term $\bu|_s\cdot \bn_s$ for $s$ in the candidate space $W^{1,\rr}(\Omega)$, as follows.  As before, let $\Omega \subset \RR^2$ be a bounded domain.  Let $[L]>0$ be a representative \emph{horizontal} scale; compare \eqref{eq:vnorm}.  For $q\in W^{1,\rr}(\Omega)$ we define
\begin{equation}
\|q\|_{W^{1,\rr}} = \left(\int_\Omega |q|^\rr\,dx + [L]^\rr \int_\Omega |\grad q|^\rr\,dx\right)^{1/\rr}. \label{eq:norm:Omega}
\end{equation}
The key assumption in the following Lemma is that $s\in W^{1,\rr}(\Omega)$ implies that the domain $\Lambda(s)$ is nice enough so that Corollary \ref{cor:surfacetracebound} gives a finite bound.  The conclusion is that we have enough control on the measurable function $\bu|_s\cdot \bn_s$ to know that it is in the dual space of $W^{1,\rr}(\Omega)$.

\begin{lemma}[Preliminary bound on surface motion] \label{lem:phibound:early}  Suppose $2 \le \rr \le \infty$, and assume $s\in W^{1,\rr}(\Omega)$ is admissible ($s\ge b$ and $s=b$ on $\partial\Omega$).  With $\Lambda(s)$ defined by \eqref{eq:icydomain}, assume the hypotheses of Theorem \ref{thm:stokeswellposed} and Corollary \ref{cor:surfacetracebound}.  Then there is $C>0$, depending on $s$ but not $q$, so that, for all  $q\in W^{1,\rr}(\Omega)$,
\begin{equation}
...
Because $\Omega\subset \RR^2$, when $\rr>2$ we have $W^{1,\rr}(\Omega) \hookrightarrow C(\bar\Omega)$.  Nonetheless, by the hypotheses of Theorem \ref{thm:stokeswellposed}, $s\in W^{1,\rr}(\Omega)$ might be insufficient regularity to solve the Stokes problem on the domain $\Lambda(s)$.  We conjecture that it suffices.

\begin{conjecture}[$W^{1,\rr}$ surface elevations allow Stokes solutions] \label{conj:a}
There exists $2 < \rr \le \infty$ so that if $s\in W^{1,\rr}(\Omega)$ is admissible then the conclusion of Theorem \ref{thm:stokeswellposed} applies on $\Lambda(s)$, providing a unique pair $(\bu,p) \in \mathcal{M}$ solving \eqref{eq:glenstokes:weak}, with $\bu\in \Vdiv$.
\end{conjecture}
...
\end{equation}
and define the closed and convex admissible subset $\cK \subset \cX$ by \eqref{eq:be:admissible}.  Assuming Conjecture \ref{conj:a}, the surface velocity $\bu|_s$ is well-defined as a measurable function (Corollary \ref{cor:surfacetracebound}).  Complementary to the above bound in Lemma \ref{lem:phibound:early}, we further conjecture that for some $\rr>2$, if $s \in \cX$ then a Lebesgue norm of $\bu|_s$ is well-behaved as a function of $s$, namely that it is \emph{Lipschitz on bounded subsets of $\cK$}.  Proving this might require more detailed knowledge about Stokes problem \eqref{eq:glenstokes:weak} than is currently available.
...
\begin{equation}
\big\|\bu|_r - \bu|_s\big\|_{L^{\rr'}} \le C(R) \|r-s\|_{W^{1,\rr}} \label{eq:ulipschitz}
\end{equation}
\end{conjecture}

From now on we will assume that Conjectures \ref{conj:a} and \ref{conj:b} holds for some $\rr>2$.
...
\begin{lemma} \label{lem:philipschitz}  The map $\Phi$ is also Lipschitz on bounded subsets of $\cK$.  For $s$, $r$, $C(R)$, and $q$ as in Conjecture \ref{conj:b},
\begin{equation}
...
At this point we have the tools needed to make the weak-form backward-Euler time step VI \eqref{eq:be:viearly} mathematically precise.  FIXME If $s\in\cK$ and $q\in\cX$ then we define $F_{\Delta t}:\cK\to\cX'$:
\begin{equation}
