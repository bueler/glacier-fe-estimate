...
For cleaner appearance we will write $s=s^n$ for the unknown surface elevation, and collect a source term, defined over all of $\Omega$, which has units of elevation:
\begin{equation}
\boxed{\ell^n(x) = s^{n-1}(x)+\Delta t\,a^n(x) = s^{n-1}(x) + \int_{t_{n-1}}^{t_n} a(t,x)\,dt.} \label{eq:be:source}
\end{equation}

As noted in the Introduction, $s$ solves a problem of free-boundary type at time $t_n$, NCP \eqref{eq:be:ncp}, which says that either there is bare ground ($s=b$) or equation \eqref{eq:be:ske} holds.  The following weak-form variational inequality (VI; \cite{Evans2010,KinderlehrerStampacchia1980}) version of this problem is better-suited to both theory and finite element (FE) analysis.  Leaving the precise Banach space $\cX$ undecided, pending the discussion below, admissible surface elevations for the weak formulation come from a convex and closed subset,
\begin{equation}
\boxed{\cK = \left\{r \in\cX\,:\,r|_{\partial\Omega}=b|_{\partial\Omega} \text{ and } r \ge b\right\}.}  \label{eq:be:admissible}
\end{equation}
Note that the fixed (Dirichlet) boundary condition is included in the definition of $\cK$.  We assume throughout, for simplicity, that $b\in C^1(\bar\Omega)$ is smooth, with bounded gradient.

The VI is derived by assuming that $s \in \cK$ is a sufficiently-regular solution of \eqref{eq:be:ncp}.  Let $\Omega_I = \{x\in\Omega\,:\,s(x)>b(x)\}$ be the (measurable) subset on which the constraint \eqref{eq:be:ncp:constraint} is inactive, i.e.~where glacier ice is present.  Integration over $\Omega_I$ gives
\begin{equation}
...
It remains to identify a Sobolev space $\cX$ suitable for admissible surface elevations, but, within a Stokes-based theory, the shape that should be predicted for a glacier's grounded margin is not clear (Figure \ref{fig:margins}).  ``Wedge'' shapes with bounded gradients have long been hypothesized \cite{EchelmeyerKamb1986}, while shallow-ice theory suggests root-type (fractional-power) shapes for the marginal surface elevation, with different shapes for advance and retreat, and unbounded gradients \cite{Bueleretal2005,JouvetBueler2012}.  However, reality is more complicated.  In the vicinity of a real glacier margin, especially on steep bedrock features, the ice can overhang, which would violate our assumption of a single-valued surface elevation function.  Also the bedrock itself can overhang.  Fractures, crevasses, and cliffs are common in margins, but modeling such features would depart from our viscous-fluid paradigm.  Because fractured margins are small compared to the overall scale of glaciers and ice sheets, major numerical models ignor overhangs, and assume instead that surface and bed elevation functions are well-defined  \cite{IsaacStadlerGhattas2015,Jouvetetal2008,LofgrenAhlkronaHelanow2022,WirbelJarosch2020}.
...
\caption{Surface elevation functions live in which Sobolev spaces?  This question relates to the shapes of ice margins.  Finite-slope ``wedge'' shapes (left) have been proposed, and fractional power shapes (center) arise in a shallow theory, but actual glacier margins have overhangs and fractures (right).}
\label{fig:margins}
\end{figure}

One might allow fractures, for example by supplementing momentum conservation with an advected damage variable \cite{PralongFunk2005}, so that fracture occurs via a stress-failure criterion.  The resulting emergent margin shapes, spatially-averaged over tens of meters, for example, might suggest an appropriate regularity assumption within a viscous-only theory, and then a Sobolev space for surface elevations.  Such a construction is clearly beyond the scope of this work.

For now we attempt to choose $\cX$ based on mathematical considerations within the viscous theory.  Observe that well-posedness of the weak-form Stokes problem \eqref{eq:glenstokes:weak}, over a 3D domain $\Lambda(s)$, plus the surface trace bound in Corollary \ref{cor:surfacetracebound}, allows us to create a well-defined map from an admissible surface elevation $s$ to the surface motion term $\bu|_s\cdot \bn_s$ in \eqref{eq:be:viearly}.  The map is defined via \eqref{eq:icydomain} for $\Lambda(s)$, followed by the solution of the Stokes model \eqref{eq:glenstokes:weak} over $\Lambda(s)$, then evaluation of the trace of $\bu$ along $\Gamma_s$ (Corollary \ref{cor:surfacetracebound}), then definition \eqref{eq:defineus} which extends by zero, and finally multiplication by $\bn_s$.  For this to be defined $s$ must be admissible ($s\ge b$) and sufficiently-regular so that the Stokes problem has a well-behaved solution with a surface trace.

The construction of this map constrains the choice of $\cX$.  We first show that the surface motion term $\bu|_s\cdot \bn_s$ in \eqref{eq:be:viearly} is in the dual space $(W^{1,\rr}(\Omega))'$ if $s\in W^{1,\rr}(\Omega)$.  Let $[L]>0$ be a representative \emph{horizontal} scale for $\Omega\subset\RR^2$.  For $q\in W^{1,\rr}(\Omega)$ define
\begin{equation}
\|q\|_{W^{1,\rr}} = \left(\int_\Omega |q|^\rr\,dx + [L]^\rr \int_\Omega |\grad q|^\rr\,dx\right)^{1/\rr}, \label{eq:norm:Omega}
\end{equation}
a norm with consistent units; compare \eqref{eq:vnorm}.  The following Lemma assumes that $s \in W^{1,\rr}(\Omega)$ is nice enough for Theorem \ref{thm:stokeswellposed} to apply.  The conclusion is that the measurable function $\bu|_s\cdot \bn_s$ is controlled, as it is an element of the dual space.

\begin{lemma}[Preliminary bound on surface motion] \label{lem:phibound:early}  Suppose $2 \le \rr \le \infty$, $s\in W^{1,\rr}(\Omega)$ is admissible ($s\ge b$ and $s=b$ on $\partial\Omega$), and $\Lambda(s)$, defined by \eqref{eq:icydomain}, satisfies the hypotheses of Theorem \ref{thm:stokeswellposed}.  Then there is $C(s)>0$ so that for all  $q\in W^{1,\rr}(\Omega)$,
\begin{equation}
\left|\int_\Omega \bu|_s\cdot \bn_s \,q\,dx\right| \le C(s)\, \|q\|_{W^{1,\rr}}. \label{eq:phibound:early}
\end{equation}
\end{lemma}
...
Because $\Omega\subset \RR^2$, if $\rr>2$ then $W^{1,\rr}(\Omega) \hookrightarrow C(\bar\Omega)$.  However, for $s\in W^{1,\rr}(\Omega)$ we do not know that $\Gamma_s$ is Lipschitz or $C^1$ as required by the hypotheses of Theorem \ref{thm:stokeswellposed}.  We conjecture that $s$ is in fact sufficiently regular.
...
For $\rr>2$ satisfying Conjecture \ref{conj:a} we define
\begin{equation}
\boxed{\cX = W^{1,\rr}(\Omega),} \label{eq:defineX}
\end{equation}
giving the admissible subset $\cK \subset \cX$ by \eqref{eq:be:admissible}.  The surface velocity $\bu|_s$ is well-defined as a measurable function, and its $L^\pp(\Gamma_s)$ norm is bounded (Corollary \ref{cor:surfacetracebound}).  The surface motion term $\bu|_s\cdot\bn_s$ is in $\cX'$ (Lemma \ref{lem:phibound:early}).
...
The abstract numerical error Theorem \ref{thm:abstractestimate} applies to VIs with operators $f:\cK\to\cX'$ which are $\qq$-coercive for $\qq>1$.  We wish to apply this to $F_{\Delta t}$ acting on $\cK\subset \cX=W^{1,\rr}(\Omega)$; see \eqref{eq:be:Fdefine}.  However, simple examples, apparently new, show that the main surface motion $\Phi$ term, namely \eqref{eq:definePhi} used in defining $F_{\Delta t}$, is \emph{not} $\qq$-coercive, nor strictly monotone, over any space $W^{1,\rr}(\Omega)$.  These examples (Appendix \ref{app:noncoercive})\footnote{The same examples negatively resolve the open question of uniqueness, for elevation-independent SMB, in the shallow ice approximation.  See Appendix \ref{app:noncoercive}.} can be constructed for any bedrock $b$ possessing local minima, by filling them with flat ice.  However, since $F_{\Delta t}$ adds the integral $\int_\Omega sq$ to the ($\Delta t$-scaled) surface motion $\Phi(s)[q]$, non-coercivity for $\Phi$ is not \emph{per se} a barrier to (strict) monotonicity of $F_{\Delta t}$.

The standard ways of writing SKE \eqref{eq:ske} as though it is an advection\footnote{References \cite{Chengetal2020,WirbelJarosch2020} specifically describe the SKE as an advection, but the idea is pervasive in Stokes model descriptions when the surface is evolved.} might also suggest that $F_{\Delta t}$ is not coercive.  The equation is often written $\frac{\partial s}{\partial t} + u \frac{\partial s}{\partial x} + v \frac{\partial s}{\partial y} = a + w$ \cite{GreveBlatter2009,SchoofHewitt2013}, with the surface velocity $\bu|_s = (u,v,w)$ in components.  However, it is not actually an advection because the surface velocity is determined through coupled stress balance equations over the domain defined by the surface elevation.  That is, $s$ and $\bu|_s$ in \eqref{eq:ske}, or its time-discretized weak form \eqref{eq:be:vi}, are found \emph{simultaneously}.  Physically, glaciers are gravity-driven viscous flows, and ice tends to flow downhill, so surface bumps are leveled and  indentations filled-in.  This large-scale diffusive response also corresponds to the infinite speed of stress transmission\footnote{Inertia is orders of magnitude smaller than viscous stresses, e.g.~a Froude number of $10^{-15}$ \cite{GreveBlatter2009}.} in a Stokes model, and it explains the smoothed appearance of actual surface elevations (Figure \ref{fig:giscross}).

On the other hand, returning to the surface motion operator $\Phi$, numerical experiments turn out to show that it is actually close to coercivity!  Appendix \ref{app:numerical} documents numerical experiments wherein thousands of high-resolution surfaces were generated, and for pairs $r,s\in \cX=W^{1,\rr}(\Omega)$ the $q$-coercivity ratios
\begin{equation}
\frac{(\Phi(r) - \Phi(s))[r - s]}{\|r-s\|_{W^{1,\rr}}^{\qq}} \label{eq:ratios}
\end{equation}
were computed, using surface velocities from the Stokes model, for exponents $\rr=4$ and $\qq=2$; compare definition \eqref{eq:qcoercive}.  If $\Phi$ were $2$-coercive over $\cX=W^{1,4}(\Omega)$, and if all computations were exact, then these ratios would all exceed $\alpha$.  From the standard presentations of Stokes models for glaciers there would seem to be little reason to expect these ratios to even be non-negative, but with mesh refinement the experimental pairs giving negative ratios become quite rare, and mean and median ratios are decidely positive, and stabilize away from zero.

FIXME regularize $w$ piece in $\Phi$ with SIA version; $\nn=3$ and fixed thickness; giving $\Phi^\eps$ which is essentially $\pp$-Laplacian; Appendix \ref{app:numerical} shows that with this addition the bad pairs disappear

\begin{conjecture}[Regularized surface motion $\Phi^\eps(s)$ is $2$-coercive over admissible surface elevations in $W^{1,4}$] \label{conj:c}  Conjectures \ref{conj:a} and \ref{conj:b} hold for $\rr=4$.  Let $\cX = W^{1,4}(\Omega)$.  Fix $b\in C^1(\bar\Omega)$ and let $\cK=\{r\in\cX\,:\,r\ge b \text{ and} r|_{\partial\Omega}=b|_{\partial\Omega}\}$.  There is $\eps>0$ so that, for $\Phi^\eps:\cK\to\cX'$ defined by \eqref{eq:definePhieps}, there is $\alpha>0$ such that
\begin{equation}
\left(\Phi^\eps(r) - \Phi^\eps(s)\right)[r-s] \ge \alpha \|r-s\|_{\cX}^2 \qquad \text{for all } r,s\in\cK. \label{eq:conj:c}
\end{equation}
\end{conjecture}

FIXME observe $\Phi^\eps$ is Lipschitz by combining Lemma \ref{lem:philipschitz} and inequality from Appendices of \cite{Bueler2021conservation}

If an operator is both continuous and $\qq$-coercive, over a closed and convex subset of a Banach space, then the corresponding VI problem is well-posed (Section \ref{sec:abstractestimate}).  To complete this section we prove that Conjecture \ref{conj:c} suffices for well-posedness of the regularization of VI problem \eqref{eq:be:vi}.

\begin{theorem} \label{thm:regularizedwellposed}  Assume Conjecture \ref{conj:c} holds for $\eps>0$.  Suppose that $s^{n-1}\in\cK$ and define the source term $\ell^n \in \cX'$ as in \eqref{eq:be:source}.  Then the regularization of operator $F_{\Delta t}$ in \eqref{eq:be:Fdefine}, $F^\eps_{\Delta t}(s)[q] = \Delta t\,\Phi^\eps(s)[q] + \int_\Omega s q$, is both continuous and $\qq$-coercive.  Thus there exists a unique surface elevation $s\in\cK$ satisfying regularized VI problem \eqref{eq:be:vi}, namely
\begin{equation}
F^\eps_{\Delta t}(s^n)[r-s^n] \ge \ell^n[r-s^n] \quad \text{for all } r \in \cK. \label{eq:regularizedvi}
\end{equation}
\end{theorem}

\begin{proof}  FIXME CHECK PROOF Let $R>0$.  By Lemma \ref{lem:philipschitz} there is $C(R)>0$ so that if $r,s\in B_R\cap\cK$ then $\big|\Phi(r)[q] - \Phi(s)[q]\big| \le C(R) \|r-s\|_{\cX} \|q\|_{\cX}$.  Then by definition \eqref{eq:be:Fdefine}, H\"older's inequality, and Sobolev's inequality we have
\begin{align}
...
Theorem \ref{thm:regularizedwellposed} addresses only the well-posedness of a single time-step, over $[t_{n-1},t_n]$.  Its conclusion is not sufficient to show well-posedness of the time-dependent problem parabolic VI problem over $[0,T]$ corresponding to NCP \eqref{eq:ncp}, nor to show that implicit steps converge in the $\Delta t\to 0$ limit, but it is a first mathematical step.  In any case, numerical models are already in use for the evolution of glacier geometry, applying a Stokes dynamics framework, and, implicit or not, their designers apparently expect their time-step problems to be well-posed, and the computed surface elevations to converge in some well-behaved manner.
