...
The well-posedness of the weak-form Stokes problem \eqref{eq:glenstokes:weak} over 3D domains $\Lambda$, and the surface trace bound in Corollary \ref{cor:surfacetracebound}, allows us to create a well-defined map from admissible surface elevations $s$ to the corresponding surface velocity solution $\bu|_s$, as long as $s\in\cK$ is sufficiently regular.  The map is defined via \eqref{eq:icydomain}, followed by the solution of \eqref{eq:glenstokes:weak} over $\Lambda$, and then evaluation of the trace of $\bu$ along $\Gamma_s$.  Note that ``$\bu|_s$'' in the current context is a function of $x$: $\bu|_s(x) = \bu(t_n,x,s(x))$.

In fact, we will define the \emph{surface motion map} $\Phi(s) = - \bu|_s\cdot \bn_s$ for this term in the SKE \eqref{eq:be:ske}.  Constructing a bound for $\Phi$ will help to identify a Banach space $\cX$ in which to seek surface elevation solutions $s$.  With this identification we will also have $\Phi:\cX \to \cX'$ where $\cX'$ is the dual space.
