...
\end{equation}
be the closure (in $\cV$) of the convex hull of the union of the constraint sets $\cK$ and $\cK_h$.  Suppose that $f$ can be extended to $\hcK$.  For $1<p<\infty$, with conjugate exponent $q=p/(p-1)$, assume $f$ is $p$-coercive over $\hcK$ with constant $\alpha>0$, and that it is Lipshitz on bounded sets of $\hcK$.  Suppose $u\in\cK$ solves \eqref{eq:vi} and $u_h\in\cK_h$ solves \eqref{eq:fe:vi}, and let $R_h=\max\{\|u\|,\|u_h\|\}$.  Then there is a constant $c=c(\alpha,R_h)>0$ so that
\begin{align}
\|u-u_h\|^p &\le \frac{2}{\alpha} \left(\inf_{v\in\cK} \ip{f(u)-\ell}{u_h-v} + \inf_{v_h\in\cK_h} \ip{f(u)-\ell}{u-v_h}\right) \label{eq:abstractestimate} \\
   &\qquad + \inf_{v_h\in\cK_h} c \|u - v_h\|^q + \frac{2}{\alpha} \ip{f(u_h)-f_h(u_h)}{u_h} \notag
\end{align}
\end{theorem}
