...
The major theorems in this paper are numerical, giving bounds on FE error in the surface elevation function (Theorems \ref{thm:abstractestimate} and \ref{thm:glacierapp}).  They extend the Falk \cite{Falk1974} to a much larger class of nonlinear operators, but the technique remains tied to the concepts of monotonicity and coercivity.
...
Our extension of the Falk bound, Theorem \ref{thm:abstractestimate}, applies to operators $f:\cK\to\cX'$ which are $\qq$-coercive for $\qq>1$.  We wish to apply this idea to the operator $F_{\Delta t}$ on $\cX=W^{1,\rr}(\Omega)$, as defined in the previous Section.  However, if this is attempted directly then it will not work.  Surprisingly simple examples show that the surface motion $\Phi$ (definition \eqref{eq:definePhi}) term in the definition of $F_{\Delta t}$ (definition \eqref{eq:be:Fdefine}) is \emph{not} coercive over any space $W^{1,\rr}(\Omega)$.  These examples can be constructed for any bedrock $b$ possessing local minima, by filling those minima with ice (Appendix \ref{app:noncoercive}).

FIXME brief: ``the surface motion term is advective'' thinking would also suggest it is not coercive

On the other hand, we seek to solve VI problem \eqref{eq:be:vi} for $F_{\Delta t}$, and it adds ``$\int_\Omega sq$'' to the (scaled) surface motion $\Phi(s)[q]$; see definition \eqref{eq:be:Fdefine}.  Because of this, the examples in Appendix \ref{app:noncoercive} are not actually a barrier to coercivity for $F_{\Delta t}$.

Furthermore, numerical experiments suggest that in fact the surface motion $\Phi$ is surprisingly close to coercivity.  Appendix \ref{app:numerical} documents our experiments wherein thousands of high-resolution surfaces were generated, and for pairs $r,s\in \cX=W^{1,\rr}(\Omega)$ the ratios
\begin{equation}
\frac{(\Phi(r) - \Phi(s))[r - s]}{\|r-s\|_{W^{1,\rr}}^{\qq}} \label{eq:ratios}
\end{equation}
were computed, specifically for $\rr=4$ and $\qq=2$.  If $\Phi$ were $2$-coercive over $\cX=W^{1,4}(\Omega)$, with constant $\alpha>0$, and if all computations were exact, then of course these ratios would always exceed $\alpha$.  From the literature of the Stokes model for glaciers there would seem to be little reason to expect these ratios to be even non-negative.  In fact, as resolution improves the surface elevation pairs where ratio \eqref{eq:ratios} comes out non-positive are quite rare in practice.
...
FIXME The error bounds proven later in Sections \ref{sec:abstractestimate} and \ref{sec:application} compare a numerical surface elevation, computed by the finite element (FE) method, with the unique solution to continuum problem \eqref{eq:be:vi}.  Of course, VI problem \eqref{eq:be:vi} must be well-posed for this comparison to make sense.  Despite the theoretical progress made in Sections \ref{sec:stokes} and \ref{sec:model}, no results known to the author prove such well-posedness, \emph{nor for any glacier geometry evolution problem based on Stokes dynamics}.  Instead we will conjecture well-posedness below.  We approach this Conjecture using comparative cases, physical reasoning, numerical experimentation, and regularization.

FIXME  We now state a mathematically-precise conjectural framework for well-posedness of this problem based upon the idea that the surface motion map $\Phi(s) = -\bu|_s\cdot \bn_s$ assigns different values to inputs which differ in Sobolev norm, and that it does so in a controlled and positive-definite manner.
