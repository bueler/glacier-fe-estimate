...
FIXME implicit BE time step VI for $s$; specific case of general fluid layer setup in \cite{Bueler2021conservation}

FIXME write out operator $\ip{f_1(s)}{w}$ or $\ip{f_2(H)}{w}$  (on $W^{1,p}(\Omega)$?); well-posedness of Stokes problem for fixed geometry \cite{JouvetRappaz2011} shows $\bu|_s$ meaningful

FIXME conjecture well-posedness of BE step for sufficiently short $\Delta t$

FIXME VI for mass continuity with $H$
