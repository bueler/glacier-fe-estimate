...
\begin{align}
F_{\Delta t}(s)[q] &= \Delta t\,\ip{\Phi(s)}{q} + \int_\Omega s q \label{eq:be:Fdefine}\\
...
\subsection{Conjectural well-posedness for the continuum problem} \label{subsec:conjecture} Subsections \ref{subsec:noflow}--\ref{subsec:margin} have deployed imperfect arguments to explain why the backward Euler VI problem \eqref{eq:be:vi} could be well-posed, or why other problems are less promising.  We now propose a mathematically-precise conjectural framework for well-posedness of \eqref{eq:be:vi} based upon a conjecture that the surface motion map $\Phi(s) = -\bu|_s\cdot \bn_s$ assigns sufficiently different results to inputs which are different in norm.
