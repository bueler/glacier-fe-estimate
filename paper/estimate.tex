...
\end{equation}
The best known example of \eqref{eq:vi} is the obstacle problem for the Laplacian operator; see \cite{Ciarlet2002,Evans2010,KinderlehrerStampacchia1980} for theory and FE analysis of that problem.  Also, \eqref{eq:regularizedvi} is in this form.

If $f:\cK \to \cX'$ is monotone and coercive (Definition \ref{def:monotonecoercive}), and continuous on finite-dimensional subspaces, then abstract VI problem \eqref{eq:vi} has a solution \cite[Corollary III.1.8]{KinderlehrerStampacchia1980}.  If $f$ is strictly monotone then the solution is unique, and if $f$ is $\qq$-coercive then it coercive and strictly monotone.  On the other hand, if $f$ is Lipschitz on bounded subsets (Definition \ref{def:lipshitz}) then it is continuous.  Recall that $f(u)-\ell \in \cX'$ is generally nonzero when $u$ solves \eqref{eq:vi}, but if $u$ is in the interior of $\cK$ then $f(u)-\ell=0$.  Under sufficient regularity assumptions, an NCP strong form generally follows from \eqref{eq:vi}.  Finally, note that Definitions \ref{def:lipshitz} and \ref{def:monotonecoercive} allow $f$ to be defined only on a subset $\cK$.

Suppose $\cX_h \subset \cX$ is a finite-dimensional subspace, typically some FE space of continuous, piecewise-polynomial functions defined on a mesh, and suppose the FE constraint set $\cK_h\subset \cX_h$ is closed and convex.  Generally $\cK_h \nsubseteq \cK$; for reasons explained below we do not assume FE states are continuum admissible.  Let $f_h:\cK_h\to\cX'$ be an approximation to $f$, and note that generally $f_h\ne f$ because of quadrature and other approximations.  In fact, in the standard FE construction of a Stokes-type glacier geometry problem (Section \ref{sec:application}), both $\cK_h \nsubseteq \cK$ and $f_h\ne f$ apply for natural reasons.
