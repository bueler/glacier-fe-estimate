...
Our primary purpose, resumed in the next Section, is to study the glacier geometry problem in NCP \eqref{eq:ncp}.  For that analysis, in Corollary \ref{cor:tracecorollary} we bound the surface trace $\bu|_s$ in terms of geometric properties of $\Lambda$.  This uses several inequalities.
...
\begin{equation}
\|\bu\|_{\cV}^\pp \le \left(\frac{\rhoi |\bg| [L]^\pp (c_{\pp}(\Lambda) + 1) k_{\pp}(\Lambda)}{2 \nu_\pp}\right)^{\pp'} |\Lambda| \label{eq:stokesapriori}
\end{equation}
...
\begin{corollary}[Surface trace bound] \label{cor:tracecorollary}
Suppose $\bu\in\cV$ is the Stokes velocity solution from Theorem \ref{thm:stokeswellposed}.  The norm of its trace over $\Gamma_s$ is controlled by geometric properties of the domain $\Lambda$, and physical constants:
\begin{equation}
\int_{\Gamma_s} |\bu|^\pp \,dS \le \frac{\gamma_{\pp}(\Lambda)}{[L]} \left(\frac{\rhoi |\bg| [L]^\pp (c_{\pp}(\Lambda) + 1) k_{\pp}(\Lambda)}{2 \nu_\pp}\right)^{\pp'} |\Lambda|. \label{eq:tracecorollary}
\end{equation}
\end{corollary}
