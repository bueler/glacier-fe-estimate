...
Now we can apply the above theory to an implicit time step of an evolving-surface, Stokes-based glacier simulation.  Our approach essentially defines the phrase ``conforming FE method'' for such glacier simulations.  We will combine the three major threads from earlier Sections: \emph{a)} well-posedness and \emph{a priori} bounds for the glaciological Stokes problem on a fixed domain (Section \ref{sec:stokes}), \emph{b)} conjectural $4$-coercivity and well-posedness of the VI problem for an implicit time step of the surface elevation (Sections \ref{sec:model} and \ref{sec:conjectural}), and \emph{c)} an abstract error estimate for the FE solutions of such VIs (Section \ref{sec:abstractestimate}).

Practical models for glaciers, whether based on stuctured or unstructured meshes, use polygonal domain geometry for the 3D Stokes problem \cite{Brinkerhoff2023,IsaacStadlerGhattas2015,Jouvetetal2008,Lengetal2012}.  For this reason we assume that the map-plane domain $\Omega \subset \RR^2$ is a polygon, and that the discrete bed and surface elevations $b_h,s_h$ are from the continuous $P_1$ FE space $\cX_h$ over a triangulation $\cT_h$ of $\Omega$ (Figure \ref{fig:fe:domain}).  The mesh need not be extruded vertically as shown in Figure \ref{fig:fe:domain}, but this is a possibility.
...
We apply monotone restriction to the original bed elevation data $b$, thus $b_h = R_h^{\oplus} b$ using \eqref{eq:monotoneop}, or in any case we assume $b_h \ge b$ and also that $b_h=b$ along $\partial\Omega$.  Now, following \eqref{eq:be:admissible}, we define $\cK_h=\{\sigma_h \in \cX_h\,:\,\sigma_h|_{\partial\Omega}=b_h|_{\partial\Omega} \text{ and } \sigma_h\ge b_h\}$, so that $\cK_h\subset\cK \subset \cX$ where $\cX = W^{1,4}(\Omega)$.

From these geometric and discrete-space choices, consider the FE method for regularised VI problem \eqref{eq:regularizedvi}, with $F^\eps_{\Delta t}$ defined by \eqref{eq:regularizedF}.  The source term $\ell^n$ in \eqref{eq:regularizedvi} may be defined exactly as in \eqref{eq:be:source}, though presumably with quadrature in the integral.  We allow the previous surface elevation to be general, i.e.~$s^{n-1} \in \cK$ and not necessarily $s^{n-1}\in\cK_h$, so the analysis to follow also applies to the first time-step, from any initial surface elevation $s^0$.

Approximate evaluation of $F^\eps_{\Delta t}(s_h)$ requires the nontrivial numerical solution of the glaciological Stokes problem \eqref{eq:glenstokes:weak} over a 3D mesh of the domain $\Lambda(s_h)$ between $z=b_h$ and $z=s_h$ (equation \eqref{eq:domainfroms} and Figure \ref{fig:fe:domain}).  We emphasize that the upper and lower surfaces of $\Lambda(s_h)$, where boundary conditions \eqref{eq:stokes:stressfreesurface} and \eqref{eq:stokes:noslide} are applied, are given by admissible FE functions $s_h,b_h\in\cX_h$, with $s_h\ge b_h$.  Solvability of this Stokes problem using inf-sup stable elements is addressed by \cite{Belenkietal2012,JouvetRappaz2011}.  The numerical velocity from solving the Stokes problem over $\Lambda(s_h)$ is denoted $\bu_h$, and its surface trace is denoted $\bu_h|_{s_h}$.  We make no restrictions to or assumptions on the solution process for $\bu_h$ other than that this surface trace is well-defined, allowing (approximate) evaluation of $F^\eps_{\Delta t}(s_h)$.  Observe that $\bu_h|_{s_h}$ will generally be different from the surface trace of the exact solution of the same Stokes boundary value problem for the same $\Lambda(s_h)$ geometry, which is denoted by $\bu|_{s_h}$. 

FIXME

For a finite-dimensional subspace $\cX_h\subset \cX$, with a constraint set $\cK_h\subset \cX_h$, we seek $s_h\in\cK_h$ solving
\begin{equation}
F^h_{\Delta t}(s_h)[r_h-s_h] \ge \ell^n[r_h-s_h] \quad \text{for all } r_h \in \cK_h. \label{eq:fe:be:vi}
\end{equation}
The operator $F^h_{\Delta t}$ denotes an FE approximation to the operator $F_{\Delta t}$; see \eqref{eq:fe:be:Fdefine} below.  

 Technically, the FE operator is defined as
\begin{equation}
