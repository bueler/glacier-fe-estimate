...
\end{equation}
The best known example of VI \eqref{eq:vi} is the obstacle problem for the Laplacian operator---see \cite{Ciarlet2002,Evans2010,KinderlehrerStampacchia1980} for theory and FE analysis---and \eqref{eq:regularizedvi} is also in this form.

Regarding this abstract VI problem, recall that $f(u)-\ell \in \cX'$ is generally nonzero when $u$ solves \eqref{eq:vi}, though if $u$ is in the interior of $\cK$ then $f(u)-\ell=0$.  Also, under sufficient regularity assumptions an NCP strong form generally follows from \eqref{eq:vi}.  Furthermore, recall that if $f:\cK \to \cX'$ is monotone and coercive (Definition \ref{def:monotonecoercive}), and continuous on finite-dimensional subspaces, then \eqref{eq:vi} has a solution \cite[Corollary III.1.8]{KinderlehrerStampacchia1980}.  If $f$ is strictly monotone then the solution is unique, and if $f$ is $\qq$-coercive then it coercive and strictly monotone.
...
For $R>0$ let $B_R = \{v\in \cX\,:\,\|v\|\le R\}$.  We say $f:\cK \to \cX'$ is \emph{Lipshitz on bounded subsets of $\cK$} if for every $R>0$ there is $C(R)>0$ so that
\begin{equation}
\|f(v)-f(w)\|_{\cX'} \le C(R) \|v-w\| \quad \text{ for all } v,w \in B_R \cap \cK.  \label{eq:liponbounded}
\end{equation}
\end{definition}

Suppose $\cX_h \subset \cX$ is a finite-dimensional subspace, typically some FE space of continuous, piecewise-polynomial functions defined on a mesh.  The FE constraint set $\cK_h\subset \cX_h$ will be assumed to be closed and convex, but generally $\cK_h \nsubseteq \cK$.  Let $f_h:\cK_h\to\cX'$, and note that generally $f_h\ne f$ because of quadrature and other approximations.\footnote{Looking ahead to Section \ref{sec:application}, both $\cK_h \nsubseteq \cK$ and $f_h\ne f$ hold in the standard FE construction of a Stokes-type glacier geometry problem.}  The FE method for \eqref{eq:vi} is the finite-dimensional VI problem
\begin{equation}
f_h(u_h)[v_h-u_h] \ge \ell[v_h-u_h] \quad \text{for all } v_h\in \cK_h. \label{eq:fe:vi}
\end{equation}
We will assume that \eqref{eq:fe:vi} has a solution $u_h\in\cK_h$, e.g.~because $f_h$ is also $\qq$-coercive.

The following \emph{a priori} theorem extends the well-known one by Falk \cite{Falk1974}; see also Theorem 5.1.1 in \cite{Ciarlet2002}.\footnote{A recent variant, where $u$ solves a variational \emph{equality}, appears in \cite[Theorem 1]{KirbyShapero2024}.}  We assume here that $f$ is coercive and Lipschitz on an (unnamed) larger set than $\cK$, which contains the FE solution $u_h$.  This technical assumption permits a clean and general result, and it reveals an extra source of error which we can avoid.  In fact, the choice of FE admissible set $\cK_h$ in Section \ref{sec:application} will avoid it by construction.  On the other hand, this Theorem does \emph{not} assume any of the following: $\cK_h \subset \cK$, $f$ is linear, $f_h=f$, $f_h$ is continuous, or $f_h$ is $\qq$-coercive.  We do not even require that $u_h$ is the \emph{unique} solution of \eqref{eq:fe:vi}; bound \eqref{eq:abstractestimate} holds for any FE solution.

\begin{theorem} \label{thm:abstractestimate}  Suppose $u\in\cK$ solves \eqref{eq:vi} and $u_h\in\cK_h$ solves \eqref{eq:fe:vi}.  For $\qq>1$, with conjugate exponent $\qq'=\qq/(\qq-1)$, assume that $f$ is $\qq$-coercive, with constant $\alpha>0$, on a set which contains $\cK$ and the numerical solution $u_h$.  Assume $f$ is also Lipschitz on bounded subsets of this larger set.  Let $R_h=\max\{\|u\|,\|u_h\|\}$.  Then there is a constant $c=c(R_h,\alpha)>0$, not otherwise depending on $u$ or $u_h$, so that
\begin{align}
...
Consider $f(u)-\ell\in \cX'$ when $u$ solves \eqref{eq:vi}.  In a unilateral obstacle problem this residual is a positive measure \cite[Theorem II.6.9]{KinderlehrerStampacchia1980}.  The first two terms in bound \eqref{eq:abstractestimate} represent errors made within its support, which is the active set.  Writing this out reveals the nature of these new, VI-specific error possibilities.  We emphasize that objects $d\mu_u$ and $A_u$ are defined from the exact solution of \eqref{eq:vi}.

\begin{corollary}  \label{cor:abstractwithmeasure}  Suppose $\cX=W^{1,\rr}(\Omega)$ is a space of scalar functions, over a domain $\Omega\subset\RR^d$, and suppose $\cK\subset\cX$ is defined as in \eqref{eq:be:admissible} for an appropriate obstacle $b$.  Let $d\mu_u$ represent the measure $f(u)-\ell\in \cX'$, and define the active set $A_u=\{x\in\Omega\,:\,u(x)>b(x)\}$.  Under the assumptions of Theorem \eqref{eq:abstractestimate} we have the bound
\begin{align}
...
On the other hand, the original Hilbert space result by Falk \cite{Falk1974} computes residual norms, losing much of the active-set information about $u$.  To make this historical connection, consider this Corollary, which applies the norm-based approach.  A standard example for the embedding is $\cX=W^{1,\rr}(\Omega)$ and $\cB=L^\rr(\Omega)$.

\begin{corollary}  \label{cor:abstractestimate:Bnorm}  Suppose that $\cX$ continuously and densely embeds into a larger Banach space $\cB$: $\cX \hookrightarrow \cB$ and $\bar{\cX} = \cB$, which implies $\cB' \subset \cX'$.  Suppose $f:\cK\to\cB'$ and $\ell\in\cB'$.  Under the assumptions of Theorem \ref{thm:abstractestimate} we have
\begin{align}
...
Falk result itself \cite{Falk1974} is recovered when $\cX$ is a Hilbert space.  One assumes that $f(v)[w]=a(v,w)$ is bilinear, uniformly-elliptic, and continuous,\footnote{The definition of uniform ellipticity coincides with definition \eqref{eq:qcoercive} of $2$-coercive, and continuity of $a(v,w)$ implies \eqref{eq:liponbounded}.} that $\cX\hookrightarrow \cH$ and $\bar{\cX} = \cH$ for some larger Hilbert space $\cH$, and finally that $f_h=f$.  In this context, up to isomorphism, $f(u)-\ell \in \cH$.  Also, since $f$ is defined on all of $\cX$, $f$ is defined on $u_h\in\cX_h\subset\cX$ so the technical assumption is unnecessary.   Then Corollary \ref{cor:abstractestimate:Bnorm} is Theorem 1 in \cite{Falk1974}.

Note that if $u$ is in the interior of $\cK$,\footnote{For glaciers this case happens only when the entire map-plane domain is covered by ice.} and if $f=f_h$, then all of these bounds \eqref{eq:abstractestimate}, \eqref{eq:abstractwithmeasure}, and \eqref{eq:abstractestimate:Bnorm} reduce to their last error term, which is Cea's lemma \cite[Theorem 2.4.1]{Ciarlet2002}, but now in a Banach space and for a coercive nonlinear operator.

The ``$\inf_{v\in\cK}$'' terms in these bounds are generally nonzero when $\cK_h \not\subset \cK$.  To see how this can occur in a unilateral obstacle problem, with $\cK$ defined as in \eqref{eq:be:admissible}, suppose $b_h=\pi_h b$ is the FE interpolant of $b$, and define $\cK_h=\{v_h \in \cX_h\,:\,v_h\ge b_h \text{ and } v_h|_{\partial\Omega}=b_h|_{\partial\Omega}\}$.  While $b_h(x_j)=b(x_j)$ for the interpolation nodes $x_j$, generally $b_h(x) \ge b(x)$ will not hold for all $x\in\Omega$ even when $b$ is arbitrarily smooth (Figure \ref{fig:nonadmissible}, left; see also \cite[Figure 5.1.3]{Ciarlet2002}).  In other words, nodal admissibility does not imply admissibility for general obstacles.

\begin{figure}[ht]
\begin{center}
\input{tikz/nonadmissible.tex} \quad \input{tikz/monoadmissible.tex}
\end{center}
\caption{Left: If $b_h=\pi_h b$ is the interpolant of $b$ then generally $\cK_h\not\subset\cK$.  Right: Defining $b_h=R_h^{\oplus} b$ instead, using monotone nodal operator \eqref{eq:monotoneop}, implies $\cK_h\subset\cK$ because $b_h\ge b$.}
\label{fig:nonadmissible}
\end{figure}

On the other hand, when $\cK_h \subset \cK$ the ``\,$\inf_{v\in\cK}$'' terms vanish.  For unilateral obstacle problems and $P_1$ or $Q_1$ elements we may force this case by using a monotone nodal operator, defined as follows.  For a node $x_i$ let $N_i$ be the union of elements adjacent to $x_i$, the \emph{star} of the node in the language of Firedrake \cite{Hametal2023}.  Define%
\begin{equation}
(R_h^{\oplus}b)(x_i) = \sup_{x \in N_i} b(x). \label{eq:monotoneop}
\end{equation}
(Compare a multilevel version in \cite{BuelerFarrell2024}.)  Let $b_h$ be the unique $P_1$/$Q_1$ function with nodal values $(R_h^{\oplus}b)(x_i)$; we write $b_h=R_h^{\oplus} b$.  Then $b_h(x)\ge b(x)$ for all $x\in \Omega$, so $\cK_h\subset \cK$ (Figure \ref{fig:nonadmissible}; right).

In summary, \emph{a priori} Theorem \ref{thm:abstractestimate} is a powerful, nontrivial generalization of Falk's bound \cite{Falk1974} and Cea's lemma.

We now return to the glacier problem, where we will use $R_h^{\oplus}$ on the bed elevation $b$.  Note that models which parameterize geometry with ice thickness, for which $\cK = \{v\in\cX\,:\,v\ge 0, v|_{\partial\Omega}=0\}$, would not need this admissibility step, assuming $P_1$/$Q_1$ elements, because then $\cK_h=\cK\cap\cX_h \subset \cK$, but we have pointed-out in the Introduction the disadvantage that the thickness tends to be less smooth than the surface.
...
Conjecture \ref{conj:c} is needed for the proposed well-posedness framework for the continuum implicit time-step problem for glaciers (Sections \ref{sec:stokes}--\ref{sec:conjectural}).  It is also critical to the application of \emph{a priori} Theorem \ref{thm:abstractestimate}, which hypothesizes coercivity, because our main goal is to bound surface elevation errors in FE simulations (Section \ref{sec:application}).  The reasonableness of the Conjecture can be tested by sampling from simulations.

The numerical experiments here, performed using Python and the Firedrake FE library \cite{Hametal2023},\footnote{Source code is at the public repository \href{https://github.com/bueler/glacier-fe-estimate}{\texttt{github.com/bueler/glacier-fe-estimate}}, in the \texttt{py/} directory.  The codes call the library at \href{https://github.com/bueler/stokes-extrude}{\texttt{github.com/bueler/stokes-extrude}}.} do not demonstrate time-stepping, implicit or otherwise.  The experiments simply generate admissible surface elevation pairs $\sigma,s\in\cK \subset \cX = W^{1,4}(\Omega)$, to use as samples in a coercivity test.  For each pair we evaluate the coercivity ratios
\begin{equation}
\rho(\sigma,s) = \frac{\left(\Phi(\sigma) - \Phi(s)\right)[\sigma-s]}{\|\sigma-s\|_{\cX}^4}. \label{eq:Phiratio}
\end{equation}
We evaluate both \eqref{eq:Phiratio} as written, using the original surface motion map defined in \eqref{eq:definePhi}, and from its regularized version $\Phi^\eps$  \eqref{eq:defineregularizedPhi} with $\eps=0.1$.

If the set of all possible ratios $\{\rho(\sigma,s)\}$ were bounded below by a positive constant $\alpha>0$, then this would confirm the coercivity conjecture, i.e.~\eqref{eq:conj:c} in the regularized case.  Obviously a numerical experiment allows only finite sampling, and particular data and discretization must be chosen anyway.  Here the domain is the 1D interval $\Omega=(-L,L)$ with $L=100$ km.  The interval $\Omega$ is uniformly-meshed into equal intervals, and $\cX_h\subset \cX$ is taken to be the continuous $P_1$ (piecewise-linear) FE space.  For $b,s\in\cX_h$, the Stokes domain $\Lambda(s)$ is therefore polygonal; see \eqref{eq:domainfroms}.

Two bed profiles (Figure \ref{fig:cases}) were considered, \emph{flat} ($b=0$) and \emph{rough} (a superposition of several wavelengths down to 4 km).  By definition \eqref{eq:be:admissible} the beds generate constraint sets $\cK_i \subset \cX$, $i=1,2$.  Three constant SMB values were considered (units $\text{m}\,\text{s}^{-1}$): $a\in\{-2.5,0.0,1.0\}\times 10^{-7}$.  For each constraint set $\cK_i$ and SMB value, a time-dependent, Stokes-dynamics run of duration $T=200$ years was simulated.

The initial surface elevation for these runs is a Halfar profile \cite{Halfar1981}, dotted in Figure \ref{fig:cases}.  For the $a=b=0$ case the exact time-dependent solution under shallow dynamics is known \cite{Halfar1981}.  Using Stokes dynamics in the numerical experiment we observed that the final surface elevation agrees closely with the shallow exact solution; compare comments in \cite{LofgrenAhlkronaHelanow2022}.  On the other hand, the nonzero SMB values caused additional advance ($a>0$ case) or retreat ($a<0$) of the ice margins, and in the latter case the glacier nearly disappeared.  Sample pairs were taken from each $\cK_i$ by extracting states at random times.

\begin{figure}[ht]
\centering
\includegraphics[width=0.35\textwidth]{figs/snapsflat.png} \quad \includegraphics[width=0.37\textwidth]{figs/snapsrough.png}
\caption{Two bed profiles (flat and rough; dashed) define constraint sets $\cK_i\subset\cX$, $i=1,2$, in the numerical experiment.  For each $\cK_i$, runs using different constant values of the SMB (see text) generated a large number of admissible surface elevations, representing large and small glaciers covering different portions of the domain; a few are shown (solid).}
\label{fig:cases}
\end{figure}

In these simulations each time-step VI was semi-implicit, approximating \eqref{eq:be:vi} using $\bu|_{s^{n-1}}\cdot\bn_{s^n}$ as mentioned in the Introduction.  The VI was then solved using a reduced-space Newton method with line search \cite{BensonMunson2006} and the stabilization technique from \cite{LofgrenAhlkronaHelanow2022}.  The Stokes problem \eqref{eq:glenstokes:weak}, with viscosity regularization $\mu_0=10^{-19}\, \text{s}^{-2}$ in \eqref{eq:glen}, was solved on each domain $\Lambda(s^{n-1})$ using a vertically-extruded mesh of quadrilaterals (Figure \ref{fig:fe:operatorvisualization}), a mixed FE method for the $Q_2\times Q_1$ (Taylor-Hood) stable pair \cite{Elmanetal2014}, and a Newton solver, with direct solution of the linear step equations.  The resulting time-dependent numerical method is only conditionally stable, but adequate for our purpose of generating sample surface elevations.

FIXME The basic result of these experiments is shown in Figure \ref{fig:ratios}.  These are sample ratio $\rho(\sigma,s)$ histograms from the highest spatial resolution, namely $\Delta x=500$ m and 40 elements in each extruded column.  More than $87\%$ of all the ratios were positive, and of these the medians for the three $\cK_i$ were in the range $[4.5,5.2] \times 10^{-13}$.  For the remaining negative ratios, the medians were in the range $[-4.1,-2.8]\times 10^{-14}$, much smaller in magnitude.
...
\caption{Histograms of ratios $\rho(\sigma,s)$ for 1000 sample pairs from each of the three sets $\cK_i$ (Figure \ref{fig:cases}).  The horizontal axis has  $\rho(\sigma,s) \in [-1,6]\times 10^{-12}$, and the common vertical axis is for counts between 0 and 350.  About $10\%$ of these ratios are negative (solid).}
\label{fig:ratios}
\end{figure}
