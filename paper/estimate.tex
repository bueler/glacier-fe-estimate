...
A different expression for the surface motion $\Phi(s)$, one which suggests coercivity, can be found for our particular Stokes model.  To state this, extend $\omega$ to $\overline{\Lambda(s)}$ as $\tilde\omega(x,z)=\omega(x)$, and consider the glaciological ice flux $\bq(s) = \int_b^s \bU\,dz$ \cite{SchoofHewitt2013}, which is the vertical integral of the horizontal portion $\bU$ of the ice velocity $\bu$.  Using incompressibility, the non-sliding assumption ($\bu|_{\Gamma_b}=\bzero$), definition \eqref{eq:definePhi}, and the divergence theorem yields
\begin{align}
\Phi(s)[\omega] &= -\int_{\Omega} \bu|_s \cdot \bn_s\,\omega\,dx = -\int_{\partial\Lambda(s)} \bu\cdot {\hat\bn}\,\tilde\omega\,dS = - \int_{\Lambda(s)} \Div\left(\tilde\omega \bu\right)  \label{eq:fluxformPhi} \\
  &= - \int_{\Lambda(s)} \bu\cdot \grad \tilde\omega = -\int_\Omega \bq(s) \cdot \grad \omega, \notag
\end{align}
where, on $\partial\Lambda(s)$, $\hat\bn$ is the outward norm unit vector and $dS$ is the area element.  FIXME

\newcommand{\wSIA}{\tilde w}
In any case, because Appendix \ref{app:noncoercive} examples show that $\Phi$ is not actually $\qq$-coercive over $W^{1,\rr}(\Omega)$, we will regularize the surface motion term using the shallow theory of ice \cite{GreveBlatter2009}.  In the limit of small aspect ratios this isothermal shallow ice approximation theory yields an expression for the surface vertical motion:
