...
\begin{align}
\|s-s_h\|_\cX^\rr &\le \quad \frac{2}{\alpha} \int_{\Omega_A(s)} (b - \ell^n) (b_h - b) &&\text{\textnormal{[term 1]}} \label{eq:glacierestimate} \\
...
\end{align}
In inequality \eqref{eq:glacierestimate},
\begin{equation}
\Gamma(s_h) = c_1 \left(\frac{\gamma_\pp(\Lambda_{s_h})}{[L]}\right)^{1/\pp} \left(|\Omega| + [L]^{-\rr}\|s_h\|_{\cX}^\rr\right)^{1/(\pp'\rr)} \|s_h\|_{L^{\pp'\rr'}}
\end{equation}
depends on $s_h$, the length scale $[L]$, and the area of $\Omega$.  The constant $c_0>0$ is from Lemma \ref{lem:preglacierapp}, and the constant $c_1>0$ depends only on the exponents $\rr$, $\pp$.
\end{theorem}
...
\medskip
\begin{itemize}
\item[term 1:] This arises from approximation of the bed in the ice-free domain defined by the exact solution $s$.  If the bed is exactly representable ($b_h=b$) then this term is zero, but generally $b_h\ge b$ and $b-\ell^n\ge 0$---see the argument in Section \ref{sec:model}---so the integrand is positive.  Note that $s_h \ge b_h \ge b = s$ in the ice-free area, the factor $b_h-b$ in the integrand is the smallest error $s_h \approx s$ which is possible here.  The presence of the coercivity constant $\alpha$ in this term shows that it relates to the ice flow in a subtle manner.

\item[term 2:] This shows how numerical errors in the numerical solution of the Stokes equations, over the domain $\Lambda_{s_h}$, affect the error in $s_h$.  For example, if a higher-order FE method were applied to the Stokes problem \eqref{eq:stokes}, so that $\|\bu_h - \bu\|_{\cV} = O(h^k)$, then this term would reflect that convergence rate.

\item[term 3:] This arises in the classical Cea's argument for convergence of FE methods \cite{Elmanetal2014}, but with added complications.  The interpolant of $s$ must be truncated into $\cK_h$, and nodal admissibility must imply admissibility.
\end{itemize}

\begin{proof}
First, apply Lemma \ref{lem:preglacierapp}.  Now consider the first term on the right side of inequality \eqref{eq:preglacierestimate}.  Because $s$ solves \eqref{eq:be:vi}, the residual $\Psi = F_{\Delta t}(s)-\ell^n \in \cX'$, while generally nonzero, is non-negative.  In particular, if $\phi\in C_c^\infty(\Omega)$ is nonnegative then $r=s+\phi \in \cK$ and $\Psi[r-s] = \Psi[\phi] \ge 0$.  Thus $\Psi\in\cX'$ is a non-negative distribution, and so it is represented by a positive Borel measure $\mu$, $\Psi[\phi] = \int_\Omega \phi\,d\mu$ \cite[Theorem 6.22]{LiebLoss1997}.  However, by the proof of Theorem II.6.9 in \cite{KinderlehrerStampacchia1980} this measure is supported in $\Omega_A(s)$.  In fact, recall from Section \ref{sec:model} that $b-\ell^n\ge 0$ on $\Omega_A(s)$---this gives the density of $\mu$--and note also that $\bu|_{s}=\bzero$ and $s=b$ on $\Omega_A(s)$.  Suppose $\phi_h \in \cX_h$ is nonnegative and supported in $\Omega_A(s)$, and define $r_h = b_h + \phi_h \in \cK_h$.  Then from definition \eqref{eq:be:Fdefine} we have
\begin{equation}
...
FIXME USE RESULT IN \cite{JouvetBueler2012}, namely \cite[Theorem 3.1.6]{Ciarlet2002}, to bound term 3
