...
Our main Theorem \ref{thm:glacierapp} is numerical, giving a bound on the error in the FE surface elevation.  It is based on an extension (Theorem \ref{thm:abstractestimate}) of the Falk \cite{Falk1974} VI technique to a large class of nonlinear operators, but this is tied to the concepts of monotonicity and coercivity.
...
\end{equation}
and \emph{strictly monotone} if equality implies $v=w$ \cite{Minty1963}.  It is \emph{coercive} \cite[Chapter III]{KinderlehrerStampacchia1980} if there is $w\in \cK$ so that $\left(f(v)-f(w)\right)[v-w]/\|v-w\| \to +\infty$ for $v \in \cK$ as $\|v\| \to +\infty$.  It is \emph{$\qq$-coercive} \cite{Bueler2021conservation} for $\qq>1$ if there exists $\alpha>0$ such that
\begin{equation}
...
Our FE numerical error theory for VIs applies to operators $f:\cK\to\cX'$ which are $\qq$-coercive for $\qq>1$.  We wish to apply this to $F_{\Delta t}$ acting on $\cK\subset \cX=W^{1,\rr}(\Omega)$; see \eqref{eq:be:Fdefine} in the previous Section.  However, simple examples, apparently new, show that the main surface motion $\Phi$ term (definition \eqref{eq:definePhi}) in $F_{\Delta t}$ is \emph{not} $\qq$-coercive, nor strictly monotone, over any space $W^{1,\rr}(\Omega)$.  These examples (Appendix \ref{app:noncoercive}) can be constructed for any bedrock $b$ possessing local minima, by filling them with flat ice; the same examples negatively resolve an open question of uniqueness in the shallow ice approximation.  However, since $F_{\Delta t}$ adds ``$\int_\Omega sq$'' to the ($\Delta t$-scaled) surface motion $\Phi(s)[q]$, non-coercivity for $\Phi$ is not \emph{per se} a barrier to (strict) monotonicity of $F_{\Delta t}$.

The standard ways of writing SKE \eqref{eq:ske} as though it is an advection\footnote{References \cite{Chengetal2020,WirbelJarosch2020} specifically describe the SKE as an advection, but the idea is pervasive in Stokes model descriptions when the surface is evolved.} might also suggest that $F_{\Delta t}$ is not coercive.  The equation is often written $\frac{\partial s}{\partial t} + u \frac{\partial s}{\partial x} + v \frac{\partial s}{\partial y} = a + w$ \cite{GreveBlatter2009,SchoofHewitt2013}, with the surface velocity $\bu|_s = (u,v,w)$ in components.  However, it is not actually an advection because the surface velocity is determined through coupled stress balance equations over the domain defined by the surface elevation; $s$ and $\bu|_s$ in \eqref{eq:ske}, or its time-discretized weak form \eqref{eq:be:vi}, are found \emph{simultaneously}.  Physically, glaciers are gravity-driven viscous flows, and ice tends to flow downhill, so surface bumps are leveled and  indentations filled-in.  This large-scale diffusive response corresponds to the infinite speed of stress transmission\footnote{Inertia is orders of magnitude smaller than viscous stresses, e.g.~a Froude number of $10^{-15}$ \cite{GreveBlatter2009}.} in a Stokes model, and it explains the smoothed appearance of actual surface elevations (Figure \ref{fig:giscross}).

Instead, returning to the surface motion operator $\Phi$, numerical experiments turn out to show that it is actually close to coercivity!  Appendix \ref{app:numerical} documents numerical experiments wherein thousands of high-resolution surfaces were generated, and for pairs $r,s\in \cX=W^{1,\rr}(\Omega)$ the $q$-coercivity ratios
\begin{equation}
...
FIXME regularize $w$ piece in $\Phi$ with SIA version; $\nn=3$ and fixed thickness; giving $\Phi^\eps$ which is essentially $\pp$-Laplacian
