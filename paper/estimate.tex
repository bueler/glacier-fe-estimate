...
\begin{align}
\|u-u_h\|^p &\le \inf_{v\in\cK} \frac{2}{\alpha} \left|\ip{f(u)-\ell}{u_h-v}\right| + \inf_{v_h\in\cK_h} \frac{2}{\alpha} \left|\ip{f(u)-\ell}{u-v_h}\right| \label{eq:abstractestimate} \\
   &\qquad + \inf_{v_h\in\cK_h} c \|u - v_h\|^q \notag
\end{align}
\end{theorem}

\begin{proof}  FIXME Let $v\in\cK$ and $v_h\in\cK_h$ be arbitrary.  Using \eqref{eq:commonextension}, rewrite VIs \eqref{eq:vi} and \eqref{eq:fe:vi}, as follows, no longer using $f_h$:
\begin{align*}
