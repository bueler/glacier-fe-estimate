...
\section{Conjectural well-posedness for the regularized implicit time-step problem} \label{sec:conjectural}

Our main Theorem \ref{thm:glacierapp} is numerical, bounding the FE error in the surface elevation function.  It is based on an extension (Theorem \ref{thm:abstractestimate}) of the Falk \cite{Falk1974} technique to a much larger class of nonlinear operators, but the extended technique is tied to the concepts of monotonicity and coercivity.
...
\end{equation}
and \emph{strictly monotone} if equality in \eqref{eq:monotone} implies $v=w$ \cite{Minty1963}.  It is \emph{coercive} \cite[Chapter III]{KinderlehrerStampacchia1980} if there is $w\in \cK$ so that $\left(f(v)-f(w)\right)[v-w]/\|v-w\| \to +\infty$ for $v \in \cK$ as $\|v\| \to +\infty$.  It is \emph{$\qq$-coercive} \cite{Bueler2021conservation} for $\qq>1$ if there exists $\alpha>0$ such that
\begin{equation}
\left(f(v)-f(w)\right)[v-w] \ge \alpha \|v-w\|^\qq \qquad \text{for all } v,w \in \cK. \label{eq:qcoercive}
\end{equation}
\end{definition}

Our extension of the Falk bound applies to operators $f:\cK\to\cX'$ which are $\qq$-coercive for $\qq>1$.  We wish to apply this idea to the operator $F_{\Delta t}$ on $\cX=W^{1,\rr}(\Omega)$; see \eqref{eq:be:Fdefine} in the previous Section.  However, attempted directly this will not work.  Simple examples, apparently new, show that the main surface motion $\Phi$ term (definition \eqref{eq:definePhi}) in $F_{\Delta t}$ is \emph{not} coercive over any space $W^{1,\rr}(\Omega)$.  These examples can be constructed for any bedrock $b$ possessing local minima, by filling them with flat ice (Appendix \ref{app:noncoercive}), along the way negatively resolving the open question of uniqueness in the shallow ice approximation.  On the other hand, the operator $F_{\Delta t}$ adds ``$\int_\Omega sq$'' to the ($\Delta t$-scaled) surface motion $\Phi(s)[q]$; see \eqref{eq:be:Fdefine}.  Because of this, examples showing non-coercivity of $\Phi$ are not \emph{per se} a barrier to monotonicity of $F_{\Delta t}$, depending on the function space.

The appearance of SKE \eqref{eq:ske}\footnote{The equation is often written $\frac{\partial s}{\partial t} + u|_s \frac{\partial s}{\partial x} + v|_s \frac{\partial s}{\partial y} = a + w|_s$ \cite{GreveBlatter2009,SchoofHewitt2013}, with $\bu|_s = (u|_s,v|_s,w|_s)$ in components.  References \cite{Chengetal2020,WirbelJarosch2020} specifically describe it as an advection, but the idea is pervasive among model descriptions.} might also suggest that $F_{\Delta t}$ is not coercive.  However, the SKE is not actually an advection because the surface velocity is determined through coupled stress balance equations over the domain defined by the surface elevation; $s$ is found \emph{simultaneously} with the ``advecting'' velocity $\bu|_s$ in \eqref{eq:ske} or its time-discretized weak form \eqref{eq:be:vi}.  Physically, glaciers are gravity-driven viscous flows, and ice flows predominantly downhill.  The surface mostly responds to perturbations with negative feedback; flow removes surface bumps and fills-in indentations.  Diffusive-in-the-large response corresponds to the infinite speed of propagation\footnote{Inertia is many orders of magnitude smaller than viscous stresses, e.g.~with a Froude number of $10^{-15}$ \cite{GreveBlatter2009}.} of boundary information in a Stokes model, and it explains the smoothed appearance of actual surface elevations (Figure \ref{fig:giscross}).

Backing up to the surface motion operator $\Phi$, numerical experiments turn out to show that it is actually close to coercivity.  Appendix \ref{app:numerical} documents our experiments wherein thousands of high-resolution surfaces were generated, and for pairs $r,s\in \cX=W^{1,\rr}(\Omega)$ the ratios
\begin{equation}
\frac{(\Phi(r) - \Phi(s))[r - s]}{\|r-s\|_{W^{1,\rr}}^{\qq}} \label{eq:ratios}
\end{equation}
were computed under the Stokes model for surface velocities, specifically for $\rr=4$ and $\qq=2$; compare \eqref{eq:qcoercive}.  If $\Phi$ were $2$-coercive over $\cX=W^{1,4}(\Omega)$, with constant $\alpha>0$, and if all computations were exact, then these ratios would all exceed $\alpha$.  From the standard presentations of Stokes models for glaciers there would seem to be little reason to expect these ratios to be even non-negative, but as the mesh resolution in these experiments was improved the surface elevation pairs with negative ratios \eqref{eq:ratios}became quite rare, and the mean and median ratios are clearly positive.

FIXME need coercivity over $W^{1,r}$ for FE error bound so regularize $w$ piece with SIA version, i.e.~$p$-Laplacian; conjecture coercivity

FIXME The error bounds proven later in Sections \ref{sec:abstractestimate} and \ref{sec:application} compare a numerical surface elevation, computed by the finite element (FE) method, with the unique solution to continuum problem \eqref{eq:be:vi}.  Of course, VI problem \eqref{eq:be:vi} must be well-posed for this comparison to make sense.  Despite the theoretical progress made in Sections \ref{sec:stokes} and \ref{sec:model}, no results known to the author prove such well-posedness, \emph{nor for any glacier geometry evolution problem based on Stokes dynamics}.  Instead we will conjecture well-posedness below.
