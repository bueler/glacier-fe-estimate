...
\begin{theorem}[Theorem 3.10 in \cite{JouvetRappaz2011}] \label{thm:stokeswellposed}  Suppose $\Lambda$ is bounded, $\partial\Lambda$ is Lipschitz, $\Gamma_s$ is $C^1$, and $\Gamma_b$ has positive measure.  Let $1<\pp\le 2$ and $\mu_0>0$ in \eqref{eq:glen}.  Then there exists a unique pair $(\bu,p) \in \mathcal{M}$ solving \eqref{eq:glenstokes:weak}, and $\bu\in \cV_0$.
\end{theorem}
...
\end{equation}
This scheme is fully-implicit because the unknown $s^n$ appears both in the surface velocity $\bu|_{s^n}$ and in the slope $\bn_{s^n}$.  For cleaner appearance, write $s=s^n$, and collect a source term, defined over all of $\Omega$, which has units of elevation:
\begin{equation}
\boxed{\ell^n(x) = s^{n-1}(x)+\Delta t\,a^n(x) = s^{n-1}(x) + \int_{t_{n-1}}^{t_n} a(t,x)\,dt.} \label{eq:be:source}
\end{equation}

As noted in the Introduction, the surface elevation $s=s^n$ actually solves a problem of free-boundary type, namely NCP \eqref{eq:be:ncp}, which says that, at the solution time $t_n$, either there is bare ground ($s=b$) or equation \eqref{eq:be:ske} holds.  The weak-form variational inequality (VI; \cite{Evans2010,KinderlehrerStampacchia1980}) version of this problem, as follows, is better-suited to both well-posedness theory and finite element (FE) analysis.  Leaving the precise Banach space $\cX$ momentarily undecided, admissible surface elevations for the weak formulation come from a convex and closed subset,
\begin{equation}
\boxed{\cK = \left\{r \in\cX\,:\,r|_{\partial\Omega}=b|_{\partial\Omega} \text{ and } r \ge b\right\};}  \label{eq:be:admissible}
\end{equation}
note this includes the fixed (Dirichlet) boundary condition.  The VI is derived by assuming that $s \in \cK$ is a sufficiently-regular solution of \eqref{eq:be:ncp} \cite{Bueler2021conservation}.  Let $\Omega_I$ be the (measurable) subset of $\Omega$ on which the $s\ge b$ constraint is inactive, where glacier ice is present: $\Omega_I = \{x\,:\,s(x)>b(x)\}$.  Integration over $\Omega_I$ then shows that
\begin{equation}
...
It remains to identify a Sobolev space $\cX$ suitable for the admissible set $\cK$ of surface elevations, but, within a Stokes-based theory anyway, the shape that should be predicted for a glacier's grounded margin is not clear (Figure \ref{fig:margins}).  Shallow-ice theory suggests root-type (fractional-power) shapes for the marginal surface elevation, with different shapes for advance and retreat, but unbounded gradients \cite{Bueleretal2005,JouvetBueler2012}.  ``Wedge'' shapes with bounded gradients have also been hypothesized \cite{EchelmeyerKamb1986}.  Reality is more complicated.  In the vicinity of an ice margin, especially on steep bedrock features, real glacier ice can generate overhangs which violate the assumption of a single-valued surface elevation function, and the bedrock itself can overhang.  Fractures, crevasses, and cliffs are also commonly found in glacier margins, but modeling such features would depart from our viscous-fluid paradigm.  However, because fractured margins are small compared to the overall scale of glaciers and ice sheets, modeling literature generally ignores overhangs, and it assumes instead that surface and bed elevation functions are well-defined  \cite{IsaacStadlerGhattas2015,Jouvetetal2008,LofgrenAhlkronaHelanow2022,WirbelJarosch2020}.  One might allow fractures, for example by supplementing momentum conservation with an additional advected damage variable \cite{PralongFunk2005}, so that ice-cliff calving occurs via a stress-failure criterion.  The resulting emergent margin shapes, spatially-averaged over tens of meters, for example, might suggest an appropriate regularity assumption within a viscous-only theory, and a Sobolev space for surface elevations, but such a construction is well outside the scope of the current work.
...
For now we attempt to choose $\cX$ based on mathematical considerations within the viscous theory.  Observe that well-posedness of the weak-form Stokes problem \eqref{eq:glenstokes:weak}, over a 3D domain $\Lambda(s)$, plus the surface trace bound in Corollary \ref{cor:surfacetracebound}, allows us to create a well-defined map from an admissible surface elevation $s$ to the surface motion term $\bu|_s\cdot \bn_s$ in \eqref{eq:be:viearly}.  The map is defined via \eqref{eq:icydomain} for $\Lambda(s)$, followed by the solution of the Stokes model \eqref{eq:glenstokes:weak} over $\Lambda(s)$, then evaluation of the trace of $\bu$ along $\Gamma_s$ (Corollary \ref{cor:surfacetracebound}), then definition \eqref{eq:defineus} which extends by zero, and finally multiplication by $\bn_s$.  Evidently, for this to be defined $s$ must be admissible ($s\ge b$) and sufficiently-regular.  The constructability of this map constrains the choice of $\cX$.

In fact, we can bound the surface motion term $\bu|_s\cdot \bn_s$ as an element of the dual space of $W^{1,\rr}(\Omega)$.  Recall $\Omega \subset \RR^2$ is a bounded domain, and let $[L]>0$ be a representative \emph{horizontal} scale.  For $q\in W^{1,\rr}(\Omega)$ define the norm
\begin{equation}
\|q\|_{W^{1,\rr}} = \left(\int_\Omega |q|^\rr\,dx + [L]^\rr \int_\Omega |\grad q|^\rr\,dx\right)^{1/\rr}; \label{eq:norm:Omega}
\end{equation}
compare \eqref{eq:vnorm}.  The key assumption in the following Lemma is that $s$ is both nice enough for Theorem \ref{thm:stokeswellposed} to apply, and in $W^{1,\rr}(\Omega)$.  The conclusion is that we have enough control on the measurable function $\bu|_s\cdot \bn_s$, the surface motion term in the SKE, so that it is in the dual space of $W^{1,\rr}(\Omega)$.
...
Because $\Omega\subset \RR^2$, when we further restrict to $\rr>2$ then we have $W^{1,\rr}(\Omega) \hookrightarrow C(\bar\Omega)$.  However, $s\in W^{1,\rr}(\Omega)$ might still have insufficient regularity to satisfy the hypotheses of Theorem \ref{thm:stokeswellposed}.  We conjecture that it suffices.
...
\end{equation}
and define the admissible subset $\cK \subset \cX$ by \eqref{eq:be:admissible}.  Assuming Conjecture \ref{conj:a}, the surface velocity $\bu|_s$ is well-defined as a measurable function, and its $L^\pp(\Gamma_s)$ norm is bounded by Corollary \ref{cor:surfacetracebound}.  The surface motion term $\bu|_s\cdot\bn_s$ is in $\cX'$ (Lemma \ref{lem:phibound:early}).

However, we must further conjecture that for some $\rr>2$, if $s \in \cX$ then a Lebesgue norm of $\bu|_s$ is well-behaved as a function of $s$, namely that it is \emph{Lipschitz on bounded subsets of $\cK$}.  Proving this might require more detailed knowledge about Stokes problem \eqref{eq:glenstokes:weak} than is currently available.
